...
MODULE correction_ms_gcs
    ...
    USE types_physical, ONLY: environ_semiconductor, environ_electrolyte
...
CONTAINS
    ...
    !!
    !! (1) ez = - tpi * e2 * charge * axis_length / omega / env_static_permittivity\f]
    !!
    ...
    !!
    !------------------------------------------------------------------------------------
    SUBROUTINE calc_vms_gcs(oned_analytic, electrolyte, semiconductor_in, charges, &
                            potential)
        !--------------------------------------------------------------------------------
        !
        IMPLICIT NONE
        ...
        !
        WRITE (environ_unit, *) &
            "bulk sc charge: ", (electrode_charge - semiconductor_in%slab_charge)
        !
        WRITE (environ_unit, *) "Mott Schottky electric field: ", ez_ms
        !
        fact = 1.D0 / tpi / e2 / 4.D0 / carrier_density
        ...
        !
        WRITE (environ_unit, *) &
            "bulk semiconductor fermi level: ", semiconductor_in%bulk_sc_fermi
        !
        DO i = 1, nnr
            !
            ...
            IF (-axis(1, i) >= xstern_ms) THEN
                ...
                IF (semiconductor_in%slab_charge == 0.D0) THEN
                    ...
                    ELSE
                        ...
                        ! Compute some constants needed for the calculation
                        !
                        fact = -e2 * SQRT(8.D0 * fpi * cion * &
                                          kbt / e2 / permittivity_gcs)
                        !
                        f1 = -fact * zion * invkbt * 0.5D0
                        f2 = 4.D0 * kbt / zion
                        !
