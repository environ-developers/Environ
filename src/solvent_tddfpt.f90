...
CONTAINS
    ...
    SUBROUTINE calc_vsolvent_tddfpt(nnr, rho_0, drho_elec, dv_pol, dv_epsilon)
        ...
        !
        CALL init_environ_electrons_second(system_cell, response_electrons)
        !
        CALL update_environ_electrons(nnr, drho_elec, response_electrons, 0.D0)
        ! END BACKWARD COMPATIBILITY
        !
        !--------------------------------------------------------------------------------
        ! Link together different sources of electrostatic potential
        ! ( charges + dielectric + electrolyte )
        !
        CALL create_environ_charges(response_charges)
        !
        CALL init_environ_charges_first(electrons=response_electrons, &
                                        dielectric=optical, charges=response_charges)
        !
        CALL init_environ_charges_second(system_cell, response_charges)
        !
        ...
        IF (lsoftsolvent) THEN
            ...
            !
            CALL init_environ_gradient(system_cell, gvtot0)
            !
            ...
            !
            CALL init_environ_gradient(system_cell, gdvtot)
            !
            ...
            DO ir = 1, nnr
                !
                dv_epsilon(ir) = &
                    -SUM(gvtot0%of_r(:, ir) * gdvtot%of_r(:, ir)) * &
                    optical%depsilon%of_r(ir) * optical%boundary%dscaled%of_r(ir) / &
                    (fpi * e2)
                !
            END DO
            !
