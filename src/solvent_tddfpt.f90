...
!--------------------------------------------------------------------
   USE environ_base, ONLY: system_cell, velectrostatic, lsoftsolvent, loptical, optical, ltddfpt
...
! END BACKWARD COMPATIBILITY
   ...
   !
   IF ( nnr .NE. system_cell % nnr ) CALL errore(sub_name,'Missmatch in passed and stored grid dimension',1)
   !
