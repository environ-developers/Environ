...
!--------------------------------------------------------------------
    ...
    IF ( solver % use_gradient ) THEN
       ...
       IF ( solver % auxiliary .EQ. 'none' ) THEN
          ...
          CASE ( 'none' )
             !
             CALL generalized_gradient_none( solver % gradient, core, charges%density, charges%dielectric, potential, charges%electrolyte, charges%semiconductor )
             !
          CASE ( 'sqrt' )
             !
             CALL generalized_gradient_sqrt( solver % gradient, core, charges%density, charges%dielectric, potential, charges%electrolyte, charges%semiconductor )
             !
          CASE ( 'left' )
             !
             CALL generalized_gradient_left( solver % gradient, core, charges%density, charges%dielectric, potential, charges%electrolyte, charges%semiconductor )
             !
          CASE DEFAULT
             !
    ...
    ELSE IF ( solver % use_iterative ) THEN
       ...
       IF ( solver % auxiliary .EQ. 'full' ) THEN
          !
          CALL generalized_iterative( solver % iterative, core, charges%density, charges%dielectric, potential, charges%electrolyte, charges%semiconductor )
          !
       ELSE
          !
          CALL errore(sub_name,'Option not yet implemented',1)
