   ...
   DO ipol = 1, 3
      ...
      DO jpol = 1, 3
         ...
         DO ig = gstart, ngm
            ...
            gaux(fft%cell%dfft%nl(ig)) = CMPLX(REAL(rhoaux(fft%cell%dfft%nl(ig))),AIMAG(rhoaux(fft%cell%dfft%nl(ig))),kind=dp) * fac 
            !
         END DO
         !
         ...
         if (do_comp_mt) then
            ...
            DO ig = gstart, ngm
               ...
               gaux(fft%cell%dfft%nl(ig)) = gaux(fft%cell%dfft%nl(ig)) + CMPLX(REAL(vaux(ig)),AIMAG(vaux(ig)),kind=dp)*fac 
            END DO
         ...
         IF ( gamma_only ) THEN
            ...
            gaux(fft%cell%dfft%nlm(:)) = &
                 CMPLX( REAL( gaux(fft%cell%dfft%nl(:)) ), -AIMAG( gaux(fft%cell%dfft%nl(:)) ) ,kind=DP)
            !
         END IF
         !
         ! ... bring back to R-space
         !
         CALL invfft ('Rho', gaux, fft%cell%dfft)
         !
 ...
 SUBROUTINE field_of_gradrho( gradrho, e, fft )
   ...
   DO ipol = 1, 3
      ...
      DO ig = gstart, ngm
         !
         fac = g(ipol,ig) / gg(ig)
         aux(fft%cell%dfft%nl(ig)) = CMPLX(-AIMAG(gaux(fft%cell%dfft%nl(ig))),REAL(gaux(fft%cell%dfft%nl(ig))),kind=dp) * fac
         !
      END DO
      !
      ! ...and add the factor e2*fpi/2\pi/a coming from the missing prefactor of
      ...
      IF ( do_comp_mt ) THEN
         ...
         DO ig = gstart, ngm
            !
            fac = g(ipol,ig) * tpiba
            aux(fft%cell%dfft%nl(ig)) = aux(fft%cell%dfft%nl(ig)) + CMPLX(-AIMAG(vaux(ig)),REAL(vaux(ig)),kind=dp)*fac
            !
         END DO
         !
      END IF
   ...
   IF ( gamma_only ) THEN
      ...
      eaux(fft%cell%dfft%nlm(:)) = &
           CMPLX( REAL( eaux(fft%cell%dfft%nl(:)) ), -AIMAG( eaux(fft%cell%dfft%nl(:)) ) ,kind=DP)
      !
   END IF
   !
   ! ... bring back to R-space, (\grad_ipol a)(r) ...
   !
   CALL invfft ('Rho', eaux, fft%cell%dfft)
   !
   e(:) = REAL( eaux(:) )
   !
   DEALLOCATE(eaux)
