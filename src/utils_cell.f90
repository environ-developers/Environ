...
CONTAINS
    ...
    !!
    !------------------------------------------------------------------------------------
    SUBROUTINE init_environ_cell(this, gcutm, comm, alat, at, nr)
        !--------------------------------------------------------------------------------
        !
        IMPLICIT NONE
        !
...
100     DO iperm = 1, 3
    ...
    SUBROUTINE print_environ_cell(this, local_verbose, local_depth)
        ...
        IF (verbosity >= 1) THEN
            ...
            END IF
            !
            WRITE (environ_unit, 1002) this%alat, this%omega
            !
            IF (verbosity >= 3) THEN
...
1001    FORMAT(/, ' CELL', /, ' ====')
        !
1002    FORMAT(/, ' lattice spacing            = ', F12.6, /, &
                ' cell volume                = ', F12.6)
        !
...
1005    FORMAT(/, ' total size of grid         = ', I12, /, &
    ...
    SUBROUTINE write_cube_cell(this, natoms)
        ...
        INTEGER :: nr1, nr2, nr3
        !
        REAL(DP), POINTER :: alat, at(:, :), origin(:)
        !
        !--------------------------------------------------------------------------------
        !
        nr1 = this%dfft%nr1
        ...
        WRITE (300, '(i5,3f12.6)') natoms, origin * alat
        WRITE (300, '(i5,3f12.6)') nr1, (at(ipol, 1) / DBLE(nr1) * alat, ipol=1, 3)
        WRITE (300, '(i5,3f12.6)') nr2, (at(ipol, 2) / DBLE(nr2) * alat, ipol=1, 3)
        WRITE (300, '(i5,3f12.6)') nr3, (at(ipol, 3) / DBLE(nr3) * alat, ipol=1, 3)
        !
        RETURN
        !
        !--------------------------------------------------------------------------------
