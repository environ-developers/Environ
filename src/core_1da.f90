...
CONTAINS
    ...
    !---------------------------------------------------------------------------
        ...
        ! (1) ez = - tpi * e2 * charge * axis_length / omega /
        ! env_static_permittivity
        !
        ! ... By integrating the Mott Schottky model, we can find that the
        ! schottky barrier
        !     is equal to
        ! (2) vms = fact*(ez)**2 + kbt
        !
        ...
        !  For more details and derivation see the appendix in Ch. 11 of
        !  Schmickler
...
solver',1)
        ...
        IF ( electrolyte % ntyp .NE. 2 ) &
             & CALL errore(sub_name,'Unexpected number of counterionic species,
different from two',1)
        ...
        IF ( env_periodicity .NE. 2 ) &
             & CALL errore(sub_name,'Option not yet implemented: 1D
           ...
           IF ( -axis(1,i) .GE. xstern_ms ) THEN
              ...
              IF (semiconductor%slab_charge .EQ. 0.D0 ) THEN
                  ...
                  IF (-axis(1,i) .GE. xstern_ms) THEN
                      ...
                      END IF
                      !
                      ! ... Remove source potential (linear) and add analytic
                      ! one
                      !
              ...
              ELSE
                  ...
                  !Only applies parabolic equation if still within the depletion
                  !width
                  !
                  IF ( distance <= depletion_length) THEN
                     !
                     ...
                     !
                     IF (ez_ms < 0.D0) THEN
                        vtmp = (distance-depletion_length)**2.D0 / fact/4.D0 +
ez_ms*(distance) + vms
...
of_r(i)
    ...
    !---------------------------------------------------------------------------
        ...
        REAL( DP ), DIMENSION(:,:), POINTER :: axis
        !
        REAL( DP ), POINTER ::  permittivity_ms, permittivity_gcs, xstern_ms,
carrier_density
...
Poisson-Boltzmann solver only for 2D systems',1)
        ...
        IF ( electrolyte % linearized ) THEN
           ...
           DO i = 1, nnr
              ...
              IF ( axis(1,i) .GE. xstern_gcs ) THEN
                 !
                 ! ... Linearized Gouy-Chapmann-Stern analytic solution on the
                 ! outside
                 !
                 arg = f1 * ABS(axis(1,i))
                 dvtmp_dx = lin_c * f1 * EXP( arg )
          ...
          IF ( -axis(1,i) .GE. xstern_ms ) THEN
              ...
              IF (semiconductor%slab_charge .EQ. 0.D0) THEN
                  IF ( electrolyte % linearized ) THEN
                     ...
                     !
                     ! ... Linearized Gouy-Chapmann-Stern analytic solution on
                     ! the outside
                     !
                     arg = f1 * ABS(axis(1,i))
                     dvtmp_dx = lin_c * f1 * EXP( arg )
                  ...
                  ELSE
                     ! ... Compute some constants needed for the calculation
                     !
                     ! these constants don't need to be redone every point of
                     ! the for loop,
                     ! what i should really do is move the if statements to the
                     ! "gcs loop"
                     fact = - e2 * SQRT( 8.D0 * fpi * cion * kbt / e2 /
permittivity_gcs )
                     f1 = - fact * zion * invkbt * 0.5D0
                     f2 = 4.D0 * kbt / zion
                     !
