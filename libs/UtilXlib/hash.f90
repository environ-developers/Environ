program env_hash
    ...
    cksum = env_fletcher32(transfer(mydata,dat),size(transfer(mydata,dat)))
    write (*,*) 'checksum of a complex array directly casting into the c function ', cksum
    call env_fletcher32_cksum(mydata,cksum)
    write (*,*) 'checksum of a complex array using fortran interface ', cksum
    call env_fletcher32_cksum(myreal,cksum)
