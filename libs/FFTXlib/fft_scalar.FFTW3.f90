...
!
  ...
  !     parameters nx,ny,nz) are stored and re-used if available
     ...
     IF( howmany /= 1 ) &
         CALL env_fftx_error__('cfft3d', ' howmany different from 1, not yet implemented for FFTW3 ', 1 )
...
SUBROUTINE env_cfft3ds (f, nx, ny, nz, ldx, ldy, ldz, howmany, isign, &
  ...
  tscale = 1.0_DP
     ...
     IF( howmany /= 1 ) &
       CALL env_fftx_error__(' cfft3ds ', ' howmany different from 1, not yet implemented for FFTW3 ', 1 )
