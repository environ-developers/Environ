...
CONTAINS
  !
  !-----------------------------------------------------------------------
  SUBROUTINE env_mp_start_images ( nimage_, parent_comm )
    !-----------------------------------------------------------------------
    !
    ...
    IF ( nimage < 1 .OR. nimage > parent_nproc ) &
       CALL env_errore( 'mp_start_images', 'invalid number of images, out of range', 1 )
    IF ( MOD( parent_nproc, nimage ) /= 0 ) &
       CALL env_errore( 'mp_start_images', 'n. of images must be divisor of nproc', 1 )
    !
    ...
    !
    CALL env_mp_comm_split ( parent_comm, my_image_id, parent_mype, &
                          intra_image_comm )
    !
    CALL env_mp_barrier( parent_comm )
    !
    ...
    !
    CALL env_mp_comm_split ( parent_comm, me_image, parent_mype, &
                         inter_image_comm )
    !
    ...
    RETURN
    !
  END SUBROUTINE env_mp_start_images
  !
  SUBROUTINE env_mp_init_image ( parent_comm )
    !
    ...
    nproc_image = env_mp_size( parent_comm )
    me_image    = env_mp_rank( parent_comm )
    !
    ...
    ionode_id = root_image
    !
  END SUBROUTINE env_mp_init_image
  !
END MODULE env_mp_images
