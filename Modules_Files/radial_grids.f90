...
END TYPE radial_grid_type
  ...
  env_do_mesh, env_check_mesh, env_hartree, env_series, &
  env_write_grid_on_file, env_read_grid_from_file, &
  env_allocate_radial_grid,&
  env_deallocate_radial_grid,&
  env_nullify_radial_grid,&
  env_radial_grid_COPY

      interface env_deallocate_radial_grid
         module procedure &
            env_deallocate_radial_grid_s,& ! only one
            env_deallocate_radial_grid_v   ! an array
      end interface
...
!
!---------------------------------------------------------------
      subroutine env_radial_grid_COPY(X,Y)
!---------------------------------------------------------------
      type(radial_grid_type),intent(in)    :: X
      type(radial_grid_type),intent(inout) :: Y
      !
      call env_deallocate_radial_grid(Y)
      call env_allocate_radial_grid(Y, X%mesh)
      !
         ...
         Y%zmesh = X%zmesh
         Y%dx    = X%dx
      end subroutine env_radial_grid_COPY
!
!---------------------------------------------------------------
      subroutine env_allocate_radial_grid(grid,mesh)
!---------------------------------------------------------------
      type(radial_grid_type),intent(inout) :: grid
      integer,intent(in) :: mesh
      if(mesh>ndmx) &
         call env_errore('allocate_radial_grid', 'mesh>ndmx',1)
      allocate(           &
         grid%r(mesh),    &
         ...
         grid%rm3(mesh)   ) ! 1 / r**3
      grid%mesh = mesh
      end subroutine env_allocate_radial_grid
!
!---------------------------------------------------------------
      subroutine env_deallocate_radial_grid_s(grid)
!---------------------------------------------------------------
      type(radial_grid_type),intent(inout) :: grid
       ...
       grid%mesh = 0
       call env_nullify_radial_grid(grid)
      end subroutine env_deallocate_radial_grid_s
!---------------------------------------------------------------
      subroutine env_deallocate_radial_grid_v(grid)
!---------------------------------------------------------------
      type(radial_grid_type),intent(inout) :: grid(:)
      integer :: n

      do n = 1,size(grid)
         call env_deallocate_radial_grid_s(grid(n))
      enddo
      
      end subroutine env_deallocate_radial_grid_v

!---------------------------------------------------------------
      subroutine env_nullify_radial_grid(grid)
!---------------------------------------------------------------
      type(radial_grid_type),intent(inout) :: grid
      nullify(           &
         grid%r,    &
         ...
         grid%rm3   ) ! 1 / r**3
      grid%mesh = -1
      end subroutine env_nullify_radial_grid
!
!---------------------------------------------------------------
      subroutine env_do_mesh(rmax,zmesh,xmin,dx,ibound,grid)
!---------------------------------------------------------------
!
      use env_kinds, only : DP
      implicit none
      ...
      !
      call env_deallocate_radial_grid(grid)
      call env_allocate_radial_grid(grid,mesh)
      !
      do i=1,mesh
         x=xmin+DBLE(i-1)*dx
      ...
      return
      end subroutine env_do_mesh
!
!  check that logarithmic grid is consistently set
!---------------------------------------------------------------
   subroutine env_check_mesh(grid)
!---------------------------------------------------------------
!
   use env_kinds, only : DP
   ...
   if (grid%mesh < 0 ) call env_errore('check_mesh','grid%mesh < 0 ',1)
   do i=1,grid%mesh
      if (abs(grid%r2(i)/grid%r(i)**2-1.d0) > eps8 ) &
         call env_errore('check_mesh',' r2(i) is different ',i)
      if (abs(grid%sqr(i)/sqrt(grid%r(i))-1.d0) > eps8 ) &
         call env_errore('check_mesh',' sqr(i) is different ',i)
      if (abs(grid%rab(i)/(grid%r(i)*grid%dx)-1.d0) > eps8 ) &
         call env_errore('check_mesh',' rab(i) is different ',i)
   end do

   return
   end subroutine env_check_mesh
!
! Solution of the Poisson's equation on a radial (logarithmic) grid
!---------------------------------------------------------------
subroutine env_hartree(k,nst,mesh,grid,f,vh)
  !---------------------------------------------------------------
  !
  use env_kinds, only : DP
!  use radial_grids, only: radial_grid_type
  ...
  !
  if (mesh.ne.grid%mesh) call env_errore('hartree',' grid dimension mismatch',1) 
  allocate(d(mesh),stat=ierr)
  allocate(e(mesh),stat=ierr)

  if (ierr.ne.0) call env_errore('hartree',' error allocating d or e',1)
  !
...
100  format(5x,'stop in "hartree": k=',i3,'  nst=',i3)
  ...
  ! not sure whether the following is really correct, but the above wasn't
     ...
     call env_series(d,grid%r,grid%r2,e(nk1))
...
!
!----------------------------------------------------------------------
subroutine env_write_grid_on_file(iunit,grid)
...
!  WRITE(iunit,'(e20.10)') (grid%rab(n), n=1,grid%mesh)
  return
end subroutine env_write_grid_on_file

subroutine env_read_grid_from_file(iunit,grid)
...
!  READ(iunit,'(e20.10)') (grid%rab(n), n=1,grid%mesh)
  ...
  return
end subroutine env_read_grid_from_file
 
!----------------------------------------------------------------------
END MODULE env_radial_grids
