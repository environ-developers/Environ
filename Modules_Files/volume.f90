...
!
!---------------------------------------------------------------------
SUBROUTINE env_volume (alat, a1, a2, a3, omega)
  !---------------------------------------------------------------------
  !
  ...
  IF ( omega < 0.0_dp) THEN
     call env_infomsg('volume','axis vectors are left-handed')
     omega = ABS (omega)
  END IF
  !
  IF ( alat < 1.0_dp) call env_infomsg('volume','strange lattice parameter')
  omega = omega * alat**3
  !
  RETURN
  !
END SUBROUTINE env_volume
