...
SUBROUTINE env_gather_real_grid ( dfft, f_in, f_out )
  ...
  do ir3 = 1, dfft%my_nr3p
    !write (6,*) 'gather grid ok 1 ir3=', ir3
     ...
     CALL MPI_GATHERV( f_in(offset_in) , recvcount(dfft%mype2), MPI_DOUBLE_PRECISION, &
                       ...
                       dfft%comm2, info )
     CALL env_fftx_error__( 'env_gather_real_grid', 'info<>0', info )
  ...
  IF( info < 0 ) &
     CALL env_fftx_error__( ' env_gather_real_grid ', ' f_out too small ', -info )
  !
  DEALLOCATE ( f_aux )
  !
...
SUBROUTINE env_gather_complex_grid ( dfft, f_in, f_out )
  ...
  do ir3 = 1, dfft%my_nr3p
     ...
     CALL MPI_GATHERV( f_in(offset_in) , recvcount(dfft%mype2), MPI_DOUBLE_PRECISION, &
                       f_aux(offset_aux), recvcount, displs, MPI_DOUBLE_PRECISION, dfft%root,      &
                       dfft%comm2, info )
     CALL env_fftx_error__( 'env_gather_complex_grid', 'info<>0', info )
     !write (6,*) 'gather grid ok 2 ir3=', ir3
     offset_in  = offset_in + dfft%nr1x * dfft%my_nr2p
     offset_aux = offset_aux + dfft%nr1x * dfft%nr2
  end do
  ...
  CALL env_mp_bcast(info, dfft%root, dfft%comm)
  IF( info < 0 ) CALL env_fftx_error__( ' env_gather_complex_grid ', ' f_out too small ', -info )
...
SUBROUTINE env_scatter_real_grid ( dfft, f_in, f_out )
  ...
  CALL env_mp_bcast(info, dfft%root, dfft%comm)
  IF( info < 0 ) CALL env_fftx_error__( ' env_scatter_real_grid ', ' f_in too small ', -info )
  info = 0
  ...
  CALL env_fftx_error__( 'env_scatter_real_grid', 'info<>0', info )

  ! 2) scatter within the comm2 communicator
  IF( size( f_out ) < dfft%nnr ) &
     CALL env_fftx_error__( ' env_scatter_real_grid ', ' f_out too small ', dfft%nnr-size( f_out ) )
  !
...
SUBROUTINE env_scatter_complex_grid ( dfft, f_in, f_out )
  ...
  CALL env_mp_bcast(info, dfft%root, dfft%comm)
  IF( info < 0 ) &
     CALL env_fftx_error__( ' env_scatter_complex_grid ', ' f_in too small ', -info )
  !
  info = 0
  !write (6,*) 'scatter grid ok 1'
  CALL MPI_SCATTERV( f_in, sendcount, displs, MPI_DOUBLE_PRECISION,   &
                     ...
                     dfft%root, dfft%comm3, info )
  !write (6,*) 'scatter grid ok 2'
  CALL env_fftx_error__( ' env_scatter_complex_grid', 'info<>0', info )

  ! 2) scatter within the comm2 communicator
  !write(*,*) 'cscatter_grid size(f_out), dfft%nnr ', size(f_out),dfft%nnr; FLUSH(6)
  IF( size( f_out ) < dfft%nnr ) &
     CALL env_fftx_error__( ' env_scatter_complex_grid ', ' f_out too small ', dfft%nnr-size( f_out ) )
  !
  displs = 0 ; f_out = 0.0D0
  DO proc =0, ( dfft%nproc2 -1 )
...
SUBROUTINE env_cgather_sym( dfftp, f_in, f_out )
  ...
  ! ... Differs from env_gather_grid because mpi_allgatherv is used instead
  ...
  IMPLICIT NONE
  !
  TYPE (env_fft_type_descriptor), INTENT(in) :: dfftp
  ...
  do ir3 = 1, dfftp%my_nr3p
     ...
     CALL MPI_ALLGATHERV( f_in(offset_in), recvcount(dfftp%mype2), MPI_DOUBLE_PRECISION, &
                          ...
                          dfftp%comm2, info )
     CALL env_fftx_error__( 'env_cgather_sym', 'info<>0', info )
...
SUBROUTINE env_cgather_sym_many( dfftp, f_in, f_out, nbnd, nbnd_proc, start_nbnd_proc )
  ...
  !
  ! ... This routine generalizes env_cgather_sym, receiveng nbnd complex
  ...
  DO proc_ = 0, dfftp%nproc - 1
     ...
     DO ibnd = 1, nbnd_proc(proc_ +1)
        ...
        do ir3 = 1, dfftp%my_nr3p
           ...
           CALL env_fftx_error__( 'env_cgather_sym_many', 'info<>0', info )
           offset_in  = offset_in  + dfftp%nr1x * dfftp%my_nr2p
           offset_aux = offset_aux + dfftp%nr1x * dfftp%nr2x
        end do
     ...
     DO ibnd = 1, nbnd_proc(proc_+1)
        ...
        do ir3 = 1, dfftp%my_nr3p
           ...
           CALL MPI_SCATTERV( f_aux(offset_aux), sendcount, displs, MPI_DOUBLE_PRECISION,   &
                              ...
                              proc2_, dfftp%comm2, info )
           CALL env_fftx_error__( 'env_gather_grid', 'info<>0', info )
