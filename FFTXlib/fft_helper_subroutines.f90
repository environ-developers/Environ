...
MODULE env_fft_helper_subroutines
  ...
  INTERFACE env_tg_reduce_rho
    MODULE PROCEDURE env_tg_reduce_rho_1,env_tg_reduce_rho_2,env_tg_reduce_rho_3,env_tg_reduce_rho_4, &
&                    env_tg_reduce_rho_5
  END INTERFACE

  INTERFACE env_c2psi_gamma
    MODULE PROCEDURE env_c2psi_gamma_cpu
#ifdef __CUDA
    MODULE PROCEDURE env_c2psi_gamma_gpu
#endif
  END INTERFACE
  PRIVATE
  PUBLIC :: env_fftx_threed2oned, env_fftx_threed2oned_gpu, env_fftx_oned2threed
  PUBLIC :: env_tg_reduce_rho
  PUBLIC :: env_tg_get_nnr, env_tg_get_recip_inc, env_fftx_ntgrp, env_fftx_tgpe, &
       env_tg_get_group_nr3
  ! Used only in CP
  PUBLIC :: env_fftx_add_threed2oned_gamma, env_fftx_psi2c_gamma, env_c2psi_gamma, &
       env_fftx_add_field, env_c2psi_gamma_tg
#if defined (__CUDA)
  PUBLIC :: env_fftx_psi2c_gamma_gpu
#endif
  PUBLIC :: env_fft_dist_info
  ! Used only in CP+EXX
  PUBLIC :: env_fftx_tgcomm
...
!     write (*,*) ' enter env_tg_reduce_rho_1'
...
!     write (*,*) ' enter env_tg_reduce_rho_2'
...
!     write (*,*) ' enter env_tg_reduce_rho_3'
  ...
  SUBROUTINE env_tg_reduce_rho_4( rhos, tmp_rhos, desc )
     ...
     INTEGER :: ierr, from, ir3, ioff, nxyp, ioff_tg
!     write (*,*) ' enter env_tg_reduce_rho_4'
  ...
  SUBROUTINE env_tg_reduce_rho_5( rhos, tmp_rhos, desc )
     ...
     INTEGER :: ierr, from, ir3, ioff, nxyp, ioff_tg
!     write (*,*) ' enter env_tg_reduce_rho_5'
...
!$omp end parallel do
  ...
  SUBROUTINE env_c2psi_gamma_cpu( desc, psi, c, ca )
     !
...
#ifdef __CUDA
  SUBROUTINE env_c2psi_gamma_gpu( desc, psi, c, ca )
     !
     !  Copy wave-functions from 1D array (c_bgrp) to 3D array (psi) in Fourier space,
     !  GPU implementation.
     !
  ...
  SUBROUTINE env_fftx_threed2oned_gpu( desc, vin_d, vout1_d, vout2_d )
     !! GPU version of \(\texttt{env_fftx_threed2oned}\).
     USE env_fft_param
     USE env_fft_types,      ONLY : env_fft_type_descriptor
     TYPE(env_fft_type_descriptor), INTENT(in) :: desc
  ...
  SUBROUTINE env_fftx_psi2c_gamma_gpu( desc, vin, vout1, vout2 )
     USE env_fft_param
     USE env_fft_types,      ONLY : env_fft_type_descriptor
     TYPE(env_fft_type_descriptor), INTENT(in) :: desc
     complex(DP), INTENT(OUT) :: vout1(:)
