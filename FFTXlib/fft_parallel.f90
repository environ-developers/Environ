...
SUBROUTINE env_tg_cft3s( f, dfft, isgn )
  ...
  ! Data are reshuffled by the env_fft_scatter_tg routine so that each of the
  ...
  USE env_fft_scatter,ONLY : env_fft_scatter_xy, env_fft_scatter_yz, &
          env_fft_scatter_tg, env_fft_scatter_tg_opt
  ...
  INTEGER                  :: i
  !
  !write (6,*) 'enter env_tg_cft3s ',isgn ; write(6,*) ; FLUSH(6)
  ...
  if (abs(isgn) == 1 ) then       ! potential fft
     ...
     CALL env_fftx_error__( ' env_tg_cft3s', ' wrong value of isgn ', 10+abs(isgn) )
  end if
  ALLOCATE( aux( nnr_ ) )
  !
  IF ( isgn > 0 ) THEN  ! G -> R
     if (isgn==+3) then
        call env_fft_scatter_tg_opt ( dfft, f, aux, nnr_, isgn)
     else
!$omp parallel do
...
!$omp end parallel do
     ...
     ! Gz, Gy, Gx
     CALL env_cft_1z( aux, nsticks_z, n3, nx3, isgn, f )
     ! Rz, Gy, Gx
     CALL env_fft_scatter_yz ( dfft, f, aux, nnr_, isgn )
     ! Gy, Gx, Rz
     CALL env_cft_1z( aux, nsticks_y, n2, nx2, isgn, f )
     ! Ry, Gx, Rz
     CALL env_fft_scatter_xy ( dfft, f, aux, nnr_, isgn )
     ! Gx, Ry, Rz
     CALL env_cft_1z( aux, nsticks_x, n1, nx1, isgn, f )
     ! Rx, Ry, Rz
  ...
  ELSE                  ! R -> G
     !
     ! Rx, Ry, Rz
     CALL env_cft_1z( f, nsticks_x, n1, nx1, isgn, aux )
     ! Gx, Ry, Rz
     CALL env_fft_scatter_xy ( dfft, f, aux, nnr_, isgn )
     ! Ry, Gx, Rz
     CALL env_cft_1z( f, nsticks_y, n2, nx2, isgn, aux )
     ! Gy, Gx, Rz
     CALL env_fft_scatter_yz ( dfft, f, aux, nnr_, isgn )
     ! Rz, Gy, Gx
     CALL env_cft_1z( f, nsticks_z, n3, nx3, isgn, aux )
     ! Gz, Gy, Gx
     if (isgn==-3) then
        call env_fft_scatter_tg_opt ( dfft, aux, f, nnr_, isgn)
     else
!$omp parallel do
        do i=1, nsticks_z*nx3
           f(i) = aux(i)
...
SUBROUTINE env_many_cft3s( f, dfft, isgn, howmany )
  ...
  USE env_fft_scalar,  ONLY : env_cft_1z
  USE env_fft_scatter, ONLY : env_fft_scatter_xy, env_fft_scatter_yz, &
         env_fft_scatter_tg_opt, env_fft_scatter_many_xy, env_fft_scatter_many_yz
  USE env_fft_types,   ONLY : env_fft_type_descriptor
  USE omp_lib
  !
  IMPLICIT NONE
  !
  TYPE (env_fft_type_descriptor), INTENT(inout), TARGET :: dfft     ! descriptor of fft data layout
  COMPLEX(DP), INTENT(inout)                        :: f( : )   ! array containing data to be transformed
  INTEGER, INTENT(in)                               :: isgn     ! fft direction (potential: +/-1, wave: +/-2, wave_tg: +/-3)
  ...
  if (abs(isgn) == 1 ) then       ! potential fft
     ...
     CALL env_fftx_error__( ' env_many_cft3s', ' Taskgroup and many not supported ', 10+abs(isgn) )
  else
     CALL env_fftx_error__( ' env_many_cft3s', ' wrong value of isgn ', 10+abs(isgn) )
  end if
  aux => dfft%aux
  !
#if defined(__FFT_OPENMP_TASKS)
  call env_execute_using_tasks()
#else
  call env_execute_using_threads()
#endif
...
#if defined(__FFT_OPENMP_TASKS)
  !
  SUBROUTINE env_execute_using_tasks()
    !
    ...
    CALL env_fftx_error__('env_many_cft3s::env_execute_using_tasks', &
    'needs thread-safe env_fft_scalar backend selected at compile time.',1)
#endif
    !
    IF ( isgn > 0 ) THEN  ! G -> R
...
!$omp do
       DO i = 0, howmany-1
          CALL env_cft_1z( f(i*nnr_+1:), nsticks_z, n3, nx3, isgn, aux(nx3*nsticks_zx*i+1:) )
       ENDDO
!$omp end do
       !
!$omp single
...
!$omp task depend(out:f(i*nnr_+1:(i+1)*nnr_))
          CALL env_cft_1z( f(i*nnr_+1:), nsticks_y, n2, nx2, isgn, aux(i*nnr_+1:), in_place=.true. )
!$omp end task
!$omp task depend(in:f(i*nnr_+1:(i+1)*nnr_)) depend(out:aux(i*nnr_+1:(i+1)*nnr_))
          comm2s = dfft%comm2s( i + 1 )
          CALL env_fft_scatter_xy( dfft, f(i*nnr_+1:), aux(i*nnr_+1:), nnr_, isgn, comm2s )
!$omp end task
!$omp task depend(in:aux(i*nnr_+1:(i+1)*nnr_))
          CALL env_cft_1z( aux(i*nnr_+1:), nsticks_x, n1, nx1, isgn, f(i*nnr_+1:) )
!$omp end task
       ENDDO
!$omp end single
       !
...
!$omp task depend(out:aux(i*nnr_+1:(i+1)*nnr_))
          CALL env_cft_1z( f(i*nnr_+1:), nsticks_x, n1, nx1, isgn, aux(i*nnr_+1:) )
!$omp end task
...
!$omp do
       DO i = 0, howmany-1
          CALL env_cft_1z( aux(nx3*nsticks_zx*i+1:), nsticks_z, n3, nx3, isgn, f(i*nnr_+1:) )
       ENDDO
!$omp end do
       !
!$omp do collapse(2)
...
!$omp end parallel
    ENDIF
  ENDSUBROUTINE env_execute_using_tasks
#endif
  !
  SUBROUTINE env_execute_using_threads()
    !
    ...
    !
#if defined(_OPENMP) && !defined(__FFT_SCALAR_THREAD_SAFE)
    CALL env_fftx_error__('env_many_cft3s::env_execute_using_threads', &
    'needs thread-safe env_fft_scalar backend selected at compile time.',1)
#endif
    !
    IF ( isgn > 0 ) THEN  ! G -> R
...
!$omp do
       DO i = 0, howmany-1
          CALL env_cft_1z( aux(i*nnr_+1:), nsticks_z, n3, nx3, isgn, f(nx3*nsticks_zx*i+1:) )
       ENDDO
!$omp end do
       !
!$omp single
...
!$omp do
       DO i = 0, howmany-1
          CALL env_cft_1z( aux(i*nnr_+1:), nsticks_y, n2, nx2, isgn, f(nx2*nsticks_yx*i+1:) )
       ENDDO
!$omp end do
       !
!$omp single
...
!$omp do
       DO i = 0, howmany-1
          CALL env_cft_1z( f(nx2*nsticks_yx*i+1:), nsticks_y, n2, nx2, isgn, aux(i*nnr_+1:))
       ENDDO
!$omp end do
       !
!$omp single
...
!$omp do
       DO i = 0, howmany-1
          CALL env_cft_1z( f(nx3*nsticks_zx*i+1:), nsticks_z, n3, nx3, isgn, aux(i*nnr_+1:) )
       ENDDO
!$omp end do
       !
!$omp do
...
!$omp end parallel
    ENDIF
  ENDSUBROUTINE env_execute_using_threads
ENDSUBROUTINE env_many_cft3s
...
SUBROUTINE env_tg_cft3s_gpu( f_d, dfft, isgn )
  ...
  USE cudafor
  !USE nvtx_fft
  USE env_fft_buffers,    ONLY : env_check_buffers_size, aux_d => dev_space_fftparallel
  USE env_fft_scalar,     ONLY : env_cft_1z_gpu
  USE env_fft_scatter_gpu,ONLY : env_fft_scatter_xy_gpu, env_fft_scatter_yz_gpu, env_fft_scatter_tg_gpu
  USE env_fft_scatter_gpu,ONLY : env_fft_scatter_tg_opt_gpu
  USE env_fft_types,  ONLY : env_fft_type_descriptor
  !
  IMPLICIT NONE
  !
  ...
  IF ( isgn > 0 ) THEN  ! G -> R
     ...
     if (isgn==+3) then 
        ...
        CALL env_cft_1z_gpu( f_d, nsticks_z, n3, nx3, isgn, aux_d, stream, in_place=.true. )
     endif
  ...
  ELSE                  ! R -> G
     ...
     if (isgn==-3) then
        ...
        call env_fft_scatter_tg_opt_gpu ( dfft, aux_d, f_d, nnr_, isgn, stream)
     else
...
SUBROUTINE env_many_cft3s_gpu( f_d, dfft, isgn, howmany )
  ...
  USE env_fft_buffers,    ONLY : env_check_buffers_size, aux_d => dev_space_fftparallel
  USE env_fft_scalar,     ONLY : env_cft_1z_gpu
  USE env_fft_scatter_gpu,ONLY : env_fft_scatter_xy_gpu, env_fft_scatter_yz_gpu, &
                              & env_fft_scatter_tg_gpu, env_fft_scatter_many_yz_gpu, &
                              & env_fft_scatter_tg_opt_gpu
  USE env_fft_types,  ONLY : env_fft_type_descriptor
  !
  IMPLICIT NONE
  !
  ...
  !
  CALL env_check_buffers_size( dfft, howmany )
  !
  ...
  IF ( isgn > 0 ) THEN  ! G -> R
     ...
     CALL env_fft_scatter_many_yz_gpu ( dfft, aux_d(1), f_d(1), howmany*nnr_, isgn, howmany )
     !
     DO i = 0, howmany-1
        CALL env_cft_1z_gpu( f_d(i*nnr_+1:), nsticks_y, n2, nx2, isgn, aux_d(i*nnr_+1:), &
                         stream, in_place=.true. ) ! dfft%stream_many((mod(i,nstreams)+1)), in_place=.true. )
     END DO
     DO i = 0, howmany-1
        CALL env_fft_scatter_xy_gpu ( dfft, f_d(i*nnr_+1:), aux_d(i*nnr_+1:), nnr_, isgn, &
                                  stream ) ! dfft%stream_many((mod(i,nstreams)+1)) )
     END DO
     !
     DO i = 0, howmany-1
        CALL env_cft_1z_gpu( aux_d(i*nnr_+1:), nsticks_x, n1, nx1, isgn, f_d(i*nnr_+1:), &
                         stream ) ! dfft%stream_many((mod(i,nstreams)+1)) )
     END DO
     !
     ! clean garbage beyond the intended dimension. should not be needed but apparently it is !
...
!$cuf kernel do(2)<<<*,*>>>
  ...
  ELSE                  ! R -> G
     ...
     END DO
     !
     CALL env_fft_scatter_many_yz_gpu ( dfft, aux_d, f_d, howmany*nnr_, isgn, howmany )
     !
     DO i = 0, howmany-1
        CALL env_cft_1z_gpu( aux_d(nx3*nsticks_zx*i+1:), nsticks_z, n3, nx3, isgn, f_d(i*nnr_+1:), &
                         stream ) ! dfft%stream_many((mod(i,nstreams)+1)) )
     END DO
     !
     ! clean garbage beyond the intended dimension. should not be needed but apparently it is !
...
!--------------------------------------------------------------------------------
!
COMPLEX (DP) FUNCTION env_get_f_of_R (i,j,k,f,dfft)
...
!
  ...
  IF ( i <= 0 .OR. i > dfft%nr1 ) CALL env_fftx_error__( ' env_get_f_of_R', ' first  index out of range ', 1 )
  IF ( j <= 0 .OR. j > dfft%nr2 ) CALL env_fftx_error__( ' env_get_f_of_R', ' second index out of range ', 2 )
  IF ( k <= 0 .OR. k > dfft%nr3 ) CALL env_fftx_error__( ' env_get_f_of_R', ' third  index out of range ', 3 )
  ...
  CALL MPI_ALLREDUCE( f_aux, env_get_f_of_R,   2, MPI_DOUBLE_PRECISION, MPI_SUM, dfft%comm, ierr )
#else
  ii = i + dfft%nr1x * (j-1) + dfft%nr1x*dfft%nr2x * (k-1)
  env_get_f_of_R = f(ii)
#endif
END FUNCTION env_get_f_of_R

SUBROUTINE env_put_f_of_R (f_in,i,j,k,f,dfft)
...
!
  ...
  IF ( i <= 0 .OR. i > dfft%nr1 ) CALL env_fftx_error__( ' env_put_f_of_R', ' first  index out of range ', 1 )
  IF ( j <= 0 .OR. j > dfft%nr2 ) CALL env_fftx_error__( ' env_put_f_of_R', ' second index out of range ', 2 )
  IF ( k <= 0 .OR. k > dfft%nr3 ) CALL env_fftx_error__( ' env_put_f_of_R', ' third  index out of range ', 3 )
...
END SUBROUTINE env_put_f_of_R

COMPLEX (DP) FUNCTION env_get_f_of_G (i,j,k,f,dfft)
...
!
  ...
  IF ( i <= 0 .OR. i > dfft%nr1 ) CALL env_fftx_error__( ' env_get_f_of_G', ' first  index out of range ', 1 )
  IF ( j <= 0 .OR. j > dfft%nr2 ) CALL env_fftx_error__( ' env_get_f_of_G', ' second index out of range ', 2 )
  IF ( k <= 0 .OR. k > dfft%nr3 ) CALL env_fftx_error__( ' env_get_f_of_G', ' third  index out of range ', 3 )
  ...
  CALL MPI_ALLREDUCE( f_aux, env_get_f_of_G,   2, MPI_DOUBLE_PRECISION, MPI_SUM, dfft%comm, ierr )
#else
  ii = i + dfft%nr1 * (j-1) + dfft%nr1*dfft%nr2 * (k-1)
  env_get_f_of_G = f(ii)
#endif
END FUNCTION env_get_f_of_G

SUBROUTINE env_put_f_of_G (f_in,i,j,k,f,dfft)
...
!
  ...
  IF ( i <= 0 .OR. i > dfft%nr1 ) CALL env_fftx_error__( ' env_put_f_of_G', ' first  index out of range ', 1 )
  IF ( j <= 0 .OR. j > dfft%nr2 ) CALL env_fftx_error__( ' env_put_f_of_G', ' second index out of range ', 2 )
  IF ( k <= 0 .OR. k > dfft%nr3 ) CALL env_fftx_error__( ' env_put_f_of_G', ' third  index out of range ', 3 )
