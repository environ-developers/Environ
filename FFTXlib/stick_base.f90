...
!=----------------------------------------------------------------------=
   ...
   PUBLIC :: env_sticks_map, env_sticks_map_allocate
...
!=----------------------------------------------------------------------=
   SUBROUTINE env_sticks_map_deallocate( smap )
      ...
      !write (6,*) ' inside env_sticks_map_deallocate'; FLUSH(6)
   ...
   ! assign 
      ...
      !write (6,*) ' inside env_sticks_map_allocate'; FLUSH(6)
      ub(1) = ( nr1 - 1 ) / 2
      ub(2) = ( nr2 - 1 ) / 2
      ub(3) = ( nr3 - 1 ) / 2
      lb    = - ub
      ...
      IF( smap%nstx == 0 ) THEN
         ...
         IF( ALLOCATED( smap%indmap ) ) THEN
            CALL env_fftx_error__(' env_sticks_map_allocate ',' indmap already allocated ', 1 )
         END IF
         IF( ALLOCATED( smap%stown ) ) THEN
            CALL env_fftx_error__(' env_sticks_map_allocate ',' stown already allocated ', 1 )
         END IF
         IF( ALLOCATED( smap%idx ) ) THEN
            CALL env_fftx_error__(' env_sticks_map_allocate ',' idx already allocated ', 1 )
         END IF
         IF( ALLOCATED( smap%ist ) ) THEN
            CALL env_fftx_error__(' env_sticks_map_allocate ',' ist already allocated ', 1 )
         END IF
      ...
      ELSE IF( smap%nstx < nstx .OR. smap%ub(3) < ub(3) ) THEN
         ...
         IF( smap%lgamma .neqv.  lgamma ) THEN
            CALL env_fftx_error__(' env_sticks_map_allocate ',' changing gamma symmetry not allowed ', 1 )
         END IF
         IF( smap%comm /= comm ) THEN
            CALL env_fftx_error__(' env_sticks_map_allocate ',' changing communicator not allowed ', 1 )
         END IF
      ...
      ELSE
         ...
         IF( smap%lgamma .neqv. lgamma ) THEN
            CALL env_fftx_error__(' env_sticks_map_allocate ',' changing gamma symmetry not allowed ', 2 )
         END IF
         IF( smap%comm /= comm ) THEN
            CALL env_fftx_error__(' env_sticks_map_allocate ',' changing communicator not allowed ', 1 )
         END IF
   ...
   SUBROUTINE env_sticks_map_set( lgamma, ub, lb, bg, gcut, st, comm )
      ...
      REAL(DP) :: amod
      INTEGER :: ngm
      !write (6,*) 'inside env_sticks_map_set gcut=',gcut; FLUSH(6)
      !write (6,*) ub,lb
...
!=----------------------------------------------------------------------=
   ...
   SUBROUTINE env_sticks_map_index( ub, lb, st, in1, in2, ngc, index_map )
      ...
      LOGICAL :: ok
      !write (6,*) ' inside env_sticks_map_index'; FLUSH(6)
      ...
      DO j2 = 0, ( ub(2) - lb(2) )
        DO j1 = 0, ( ub(1) - lb(1) )
          ...
          IF( st( i1, i2 ) > 0 ) THEN
            ...
            ind = index_map( i1, i2 )
            IF( nct > min_size ) &
              CALL env_fftx_error__(' env_sticks_map_index ',' too many sticks ', nct )
            in1(ind) = i1
            in2(ind) = i2
            ngc(ind) = st( i1 , i2)
          ENDIF
...
!=----------------------------------------------------------------------=
   ...
   SUBROUTINE env_sticks_dist_new( lgamma, mype, nproc, nyfft, iproc, iproc2, ub, lb, idx, in1, in2, ngc, nct, ncp, ngp, stown, ng )
      ...
      !write (6,*) 'nyfft inside env_sticks_dist_new', nyfft
      ...
      DO mc =1, nct
         ...
         IF (stown(i1,i2) > 0) THEN
            ...
            if (ygr(i1) .ne. gr ) CALL env_fftx_error__(' sticks_dist ',' ygroups are not compatible ', 1 )
         END IF
      END DO
      do i1 = lb(1),ub(1)
         if (ygr(i1) == 0 ) cycle
      ...
      !write (6,*) 'inside env_sticks_dist_new', lb(1),ub(1)
...
!---------------------------------------------------------------------
   ...
   SUBROUTINE env_get_sticks( smap, gcut, nstp, sstp, st, nst, ng )
      ...
      INTEGER :: ic
      !write (6,*) ' inside env_get_sticks gcut=',gcut; FLUSH(6)
      !write (6,*) smap%lb, smap%ub

      IF( .NOT. ALLOCATED( smap%stown ) ) THEN
         CALL env_fftx_error__(' env_get_sticks ',' sticks map, not allocated ', 1 )
      END IF
