...
!=----------------------------------------------------------------------=!
   ...
   SUBROUTINE env_initialize_threads()
      implicit none
#if defined(_OPENMP)
      integer :: fftw_return
      ...
      if (.not. threads_initialized) then
         ...
         if (fftw_return == 0) then
            call env_fftx_error__(" env_fft_scalar_fftw3::env_initialize_threads ", &
...
#endif
   END SUBROUTINE env_initialize_threads
...
!
  ...
  !     parameters nx,ny,nz) are stored and re-used if available
     ...
     IF( howmany /= 1 ) &
         CALL env_fftx_error__('env_cfft3d', ' howmany different from 1, not yet implemented for FFTW3 ', 1 )
   ...
   CONTAINS
     ...
     SUBROUTINE env_init_plan()
       ...
       IF ( nx /= ldx .or. ny /= ldy .or. nz /= ldz ) &
            call env_fftx_error__('cfft3','not implemented',3)
...
SUBROUTINE env_cfft3ds (f, nx, ny, nz, ldx, ldy, ldz, howmany, isign, &
  ...
  tscale = 1.0_DP
     ...
     IF( howmany /= 1 ) &
       CALL env_fftx_error__(' env_cfft3ds ', ' howmany different from 1, not yet implemented for FFTW3 ', 1 )
