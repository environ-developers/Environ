...
!=----------------------------------------------------------------------=!
        ...
        PUBLIC :: env_fft_scatter_gpu, env_fft_scatter_gpu_batch
        PUBLIC :: env_fft_scatter_many_columns_to_planes_send, &
...
!----------------------------------------------------------------------------
SUBROUTINE env_fft_scatter_gpu ( dfft, f_in_d, f_in, nr3x, nxx_, f_aux_d, f_aux, ncp_, npp_, isgn )
  !
  USE cudafor
  IMPLICIT NONE
  !
...
!$cuf kernel do(2) <<<*,*>>>
        ...
        if( istat ) CALL env_fftx_error__("env_fft_scatter", "ERROR cudaMemcpy2D failed : ", istat)
#endif
...
END SUBROUTINE env_fft_scatter_gpu

SUBROUTINE env_fft_scatter_gpu_batch ( dfft, f_in_d, f_in, nr3x, nxx_, f_aux_d, f_aux, ncp_, npp_, isgn, batchsize, srh )
  !
  ! This subroutine performs the same task as env_fft_scatter_gpu, but for
...
!$cuf kernel do(2) <<<*,*>>>
        ...
        IF( istat /= cudaSuccess ) CALL env_fftx_error__ ('env_fft_scatter', 'cudaMemcpy2D failed: ', istat)
#endif
        !
        offset = offset + npp_ ( gproc )
     ENDDO
...
#ifdef __GPU_MPI
     istat = cudaMemcpyAsync( f_in_d( (me-1)*sendsiz + 1), f_aux_d((me-1)*sendsiz + 1), sendsiz, stream=dfft%a2a_comp )
     IF( istat /= cudaSuccess ) CALL env_fftx_error__ ('env_fft_scatter', 'cudaMemcpyAsync failed: ', istat)
#else
...
END SUBROUTINE env_fft_scatter_gpu_batch

SUBROUTINE env_fft_scatter_many_columns_to_planes_store ( dfft, f_in_d, f_in, nr3x, nxx_, f_aux_d, f_aux, f_aux2_d, f_aux2, ncp_, npp_, isgn, batchsize, batch_id )
   !
   USE cudafor
   IMPLICIT NONE
   !
  ...
  call get_ipc_peers( dfft%IPC_PEER )
   ...
   ierr    = 0
   !
   IF (isgn.lt.0) CALL env_fftx_error__ ('env_fft_scatter_many_columns_to_planes_store', 'isign is wrong', isgn )
   !
   IF (nprocp==1) GO TO 10
   !
   ! "forward" scatter from columns to planes
   ...
   DO iter = 2, nprocp
      ...
      IF( istat /= cudaSuccess ) CALL env_fftx_error__ ('env_fft_scatter_many_columns_to_planes_store', 'cudaMemcpy2DAsync failed : ', istat)
#else
#ifdef __IPC
...
SUBROUTINE env_fft_scatter_many_columns_to_planes_send ( dfft, f_in_d, f_in, nr3x, nxx_, f_aux_d, f_aux, f_aux2_d, f_aux2, ncp_, npp_, isgn, batchsize, batch_id )
   !
   USE cudafor
   IMPLICIT NONE
   !
   ...
   !
   ierr = 0
   IF (isgn.lt.0) CALL env_fftx_error__ ('env_fft_scatter_many_columns_to_planes_send', 'isign is wrong', isgn )
...
SUBROUTINE env_fft_scatter_many_planes_to_columns_store ( dfft, f_in_d, f_in, nr3x, nxx_, f_aux_d, f_aux, f_aux2_d, f_aux2, ncp_, npp_, isgn, batchsize, batch_id )
   !
   USE cudafor
   IMPLICIT NONE
   !
   ...
   ierr = 0
   !
   IF (isgn.gt.0) CALL env_fftx_error__ ('env_fft_scatter_many_planes_to_columns_store', 'isign is wrong', isgn )
   !
   !
   !  "backward" scatter from planes to columns
   !
...
SUBROUTINE env_fft_scatter_many_planes_to_columns_send ( dfft, f_in_d, f_in, nr3x, nxx_, f_aux_d, f_aux, f_aux2_d, f_aux2, ncp_, npp_, isgn, batchsize, batch_id )
   !
   USE cudafor
   IMPLICIT NONE
   !
   ...
   ierr = 0
   IF (isgn.gt.0) CALL env_fftx_error__ ('env_fft_scatter_many_planes_to_columns_send', 'isign is wrong', isgn )
...
!         END DO
        ...
        cudaMemcpyDeviceToDevice, dfft%bstreams(batch_id) )
        IF( istat /= cudaSuccess ) CALL env_fftx_error__ ('env_fft_scatter_many_planes_to_columns_send', 'cudaMemcpy2DAsync failed : ', istat)
...
!
!=----------------------------------------------------------------------=!
END MODULE env_fft_scatter_2d_gpu
!=----------------------------------------------------------------------=!
#endif
