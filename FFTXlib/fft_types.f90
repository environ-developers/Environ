...
!=----------------------------------------------------------------------------=!
  ...
  PUBLIC :: env_fft_type_descriptor, env_fft_type_init
  PUBLIC :: env_fft_type_allocate, env_fft_type_deallocate
  PUBLIC :: env_fft_stick_index, env_fft_index_to_3d
...
!=----------------------------------------------------------------------------=!
  ...
  !
    ...
    INTEGER :: color, key
    !write (6,*) ' inside env_fft_type_allocate' ; FLUSH(6)

    desc%comm = comm
#if defined(__MPI)
    IF( desc%comm == MPI_COMM_NULL ) THEN
       CALL env_fftx_error__( ' env_fft_type_allocate ', ' fft communicator is null ', 1 )
    END IF
#endif
    !
    IF ( ALLOCATED( desc%nsp ) ) &
        CALL env_fftx_error_uniform__(' env_fft_type_allocate ', ' fft arrays already allocated ', 1, desc%comm )
    ...
    IF ( present(nyfft) ) THEN
      ...
      CALL env_fftx_error__( ' env_fft_type_allocate ', ' MOD(nproc,nyfft) .ne. 0 ', MOD(nproc,nyfft) )
...
!#define ZCOMPACT
    ...
    DO i = 1, desc%nstream_many
        ...
        IF ( ierr /= 0 ) CALL env_fftx_error__( ' env_fft_type_allocate ', ' Error creating stream ', i )
    END DO
  ...
  SUBROUTINE env_fft_type_deallocate( desc )
    ...
    INTEGER :: i, ierr, nsubbatches
     !write (6,*) ' inside env_fft_type_deallocate' ; FLUSH(6)
    ...
    IF (desc%a2a_comp /= 0) THEN 
      ...
      CALL env_fftx_error__("env_fft_type_deallocate","failed destroying stream a2a_comp", ierr)
      desc%a2a_comp = 0
    END IF 
...
!=----------------------------------------------------------------------------=!
  ...
  SUBROUTINE env_fft_type_set( desc, nst, ub, lb, idx, in1, in2, ncp, ncpw, ngp, ngpw, st, stw, nmany )
    ...
    INTEGER :: ierr
     !write (6,*) ' inside env_fft_type_set' ; FLUSH(6)
    !
    !
#if defined(__MPI)
#if defined(__FFT_OPENMP_TASKS)
    ...
    IF (.NOT. ALLOCATED( desc%nsp ) ) &
        CALL env_fftx_error__(' env_fft_type_set ', ' fft arrays not yet allocated ', 1 )

    IF ( desc%nr1 == 0 .OR. desc%nr2 == 0 .OR. desc%nr3 == 0 ) &
        CALL env_fftx_error__(' env_fft_type_set ', ' fft dimensions not yet set ', 1 )
    ...
    IF( ( nr1 > nr1x ) .or. ( nr2 > nr2x ) .or. ( nr3 > nr3x ) ) &
      CALL env_fftx_error__( ' env_fft_type_set ', ' wrong fft dimensions ', 1 )
    ...
    IF( ( size( desc%ngl ) < desc%nproc ) .or.  ( size( desc%iss ) < desc%nproc ) .or. &
      ...
      CALL env_fftx_error__( ' env_fft_type_set ', ' wrong descriptor dimensions ', 2 )

    IF( ( size( idx ) < nst ) .or. ( size( in1 ) < nst ) .or. ( size( in2 ) < nst ) ) &
      CALL env_fftx_error__( ' env_fft_type_set ', ' wrong number of stick dimensions ', 3 )

    IF( ( size( ncp ) < desc%nproc ) .or. ( size( ngp ) < desc%nproc ) ) &
      CALL env_fftx_error__( ' env_fft_type_set ', ' wrong stick dimensions ', 4 )
...
!!!!!!!
    ...
    IF( size( desc%isind ) < ( nr1x * nr2x ) ) &
      CALL env_fftx_error__( ' env_fft_type_set ', ' wrong descriptor dimensions, isind ', 5 )

    IF( size( desc%iplp ) < ( nr1x ) .or. size( desc%iplw ) < ( nr1x ) ) &
      CALL env_fftx_error__( ' env_fft_type_set ', ' wrong descriptor dimensions, ipl ', 5 )

    IF( desc%my_nr3p == 0 .and. ( .not. desc%use_pencil_decomposition ) ) &
      CALL env_fftx_error__( ' env_fft_type_set ', &
    ...
    do m1=1,desc%nr1x
       if (desc%iplw(m1)>0) then
          if (desc%iplp(m1) /= desc%iplw(m1) )  then
             ...
             write (6,*) desc%iplp
             write (6,*) desc%iplw
             CALL env_fftx_error__( ' env_fft_type_set ', ' iplp is wrong ', m1 )
          end if
       end if
    end do
    ...
    do i1 = 1, nr1
       if ( (desc%iplw(i1) > 0) .and. (desc%iplp(i1) == 0) ) &
             CALL env_fftx_error__( ' env_fft_type_set ', ' bad distribution of X values ', i1 )
    ...
    IF( size( desc%ismap ) < ( nst ) ) &
      CALL env_fftx_error__( ' env_fft_type_set ', ' wrong descriptor dimensions ', 6 )
    ...
    IF( any( nsp( 1:desc%nproc ) /= ncpw( 1:desc%nproc ) ) ) THEN
      ...
      ENDDO
      CALL env_fftx_error__( ' env_fft_type_set ', ' inconsistent number of sticks ', 7 )
    ENDIF
    ...
    IF( any( nsp( 1:desc%nproc ) /= ncp( 1:desc%nproc ) ) ) THEN
      ...
      ENDDO
      CALL env_fftx_error__( ' env_fft_type_set ', ' inconsistent number of sticks ', 8 )
    ENDIF
    ...
    !write (6,*) 'env_fft_type_set SUMMARY'
...
!=----------------------------------------------------------------------------=!
  ...
  SUBROUTINE env_fft_type_init( dfft, smap, pers, lgamma, lpara, comm, at, bg, gcut_in, dual_in, fft_fact, nyfft, nmany, use_pd )
...
! ...   nstw     local number of sticks (wave functions)
     ...
     INTEGER  :: ngm, ngw
     !write (6,*) ' inside env_fft_type_init' ; FLUSH(6)
     ...
     ELSE IF ( pers == 'wave' ) THEN
        ...
        gcut = gkcut * dual
     ELSE
        CALL env_fftx_error__(' env_fft_type_init ', ' unknown FFT personality ', 1 )
     END IF
     !write (*,*) 'env_fft_type_init pers, gkcut,gcut', pers, gkcut, gcut
     ...
     ELSE
        IF( dfft%comm /= comm ) THEN
           CALL env_fftx_error__(' env_fft_type_init ', ' FFT already allocated with a different communicator ', 1 )
        END IF
     END IF

     IF ( PRESENT (use_pd) ) dfft%use_pencil_decomposition = use_pd
     IF ( ( .not. dfft%use_pencil_decomposition ) .and. ( nyfft > 1 ) ) &
        CALL env_fftx_error_uniform__(' env_fft_type_init ', ' Slab decomposition and task groups not implemented. ', 1, dfft%comm )
     ...
     !write(*,*) 'calling env_get_sticks with gkcut =',gkcut
     CALL env_get_sticks(  smap, gkcut, nstpw, sstpw, stw, nstw, ngw )
     !write(*,*) 'calling env_get_sticks with gcut =',gcut
     ...
     IF( dfft%ngw /= ngw ) THEN
        CALL env_fftx_error__(' env_fft_type_init ', ' wrong ngw ', 1 )
     END IF
     IF( dfft%ngm /= ngm ) THEN
        CALL env_fftx_error__(' env_fft_type_init ', ' wrong ngm ', 1 )
     END IF
...
!=----------------------------------------------------------------------------=!
     ...
     SUBROUTINE env_realspace_grid_init( dfft, at, bg, gcutm, fft_fact )
       ...
       INTEGER, INTENT(IN), OPTIONAL :: fft_fact(3)
       TYPE(env_fft_type_descriptor), INTENT(INOUT) :: dfft
     !write (6,*) ' inside env_realspace_grid_init' ; FLUSH(6)
       !
...
! ... declare other variables
      INTEGER :: i, j, k, nb(3)
      REAL(DP) :: gsq, g(3)
     !write (6,*) ' inside env_grid_set' ; FLUSH(6)
...
! ... the following choice, the sphere in G-space "touches" its periodic image
   ...
   !
   SUBROUTINE env_fft_index_to_3d (ir, dfft, i,j,k, offrange)
     !
     ...
     offrange = (i < 0 .OR. i >= dfft%nr1 ) .OR. &
          ...
          (k < 0 .OR. k >= dfft%nr3 )
     !
   END SUBROUTINE env_fft_index_to_3d
