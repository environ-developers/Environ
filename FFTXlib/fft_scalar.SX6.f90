...
!
  ...
  !     parameters nx,ny,nz) are stored and re-used if available
     ...
     IF ( howmany /= 1 ) &
         call env_fftx_error__('env_cfft3d',' homany different from 1, not yet implemented for SX6 ', 1)
     ...
     IF( ip == -1 ) THEN
       ...
       IF (err /= 0) CALL env_fftx_error__('env_cfft3d','FFT init returned an error ', err)
...
!$omp end parallel do
     ...
     IF (err /= 0) CALL env_fftx_error__('env_cfft3d','FFT returned an error ', err)
     DEALLOCATE(cw2)
