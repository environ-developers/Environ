...
!     parameters nz, nsl, ldz) are stored and re-used if available
     ...
     IF( nsl < 0 ) THEN
       CALL env_fftx_error__(" env_fft_scalar: env_cft_1z ", " nsl out of range ", nsl)
     END IF
     ...
     IF (isign < 0) THEN
        ...
        call env_fftx_error__(" env_fft_scalar_cuFFT: env_cft_1z_gpu ", " cufftExecZ2Z failed ", istat)
        tscale = 1.0_DP / nz
        IF (is_inplace) THEN
...
!$cuf kernel do(1) <<<*,*,0,stream>>>
     ...
     SUBROUTINE env_init_plan()
       ...
       IF( cufft_planz( icurrent) /= 0 ) THEN
           ...
           call env_fftx_error__(" env_fft_scalar_cuFFT: env_cft_1z_gpu ", " cufftDestroy failed ", istat)
       ENDIF
       ...
       istat = cufftPlanMany( cufft_planz( icurrent), RANK, FFT_DIM, &
                              ...
                              DATA_DIM, STRIDE, DIST, &
                              CUFFT_Z2Z, BATCH )
       call env_fftx_error__(" env_fft_scalar_cuFFT: env_cft_1z_gpu ", " cufftPlanMany failed ", istat)
...
!pgi$ ignore_tkr r_d, temp_d
     ...
     IF( PRESENT( pl2ix ) ) THEN
       IF( SIZE( pl2ix ) < nx ) &
         CALL env_fftx_error__( ' env_cft_2xy ', ' wrong dimension for arg no. 8 ', 1 )
       DO i = 1, nx
...
!         print *,"batch: ",batch_start,batch_end,batch_count
     ...
     call env_fftx_error__(" env_fft_scalar_cuFFT: env_cft_2xy_gpu ", " failed to set stream ", istat)
#else
     ...
     IF( isign < 0 ) THEN
        ...
        call env_fftx_error__(" env_fft_scalar_cuFFT: env_cft_2xy_gpu ", " cufftExecZ2Z failed ", istat)
!$cuf kernel do(3) <<<*,(16,16,1), 0, stream>>>
        ...
        call env_fftx_error__(" env_fft_scalar_cuFFT: env_cft_2xy_gpu ", &
...
!$cuf kernel do(3) <<<*,(16,16,1), 0, stream>>>
        ...
        if(batch_1>0) then
           ...
           call env_fftx_error__(" env_fft_scalar_cuFFT: env_cft_2xy_gpu ", " in fftxy ffty batch_1 istat = ", istat)
        end if

        if(batch_2>0) then
           istat = cufftExecZ2Z( cufft_plan_y(2,ip), temp_d(1,1,nx-batch_2+1), temp_d(1,1,nx-batch_2+1), CUFFT_INVERSE )
           call env_fftx_error__(" env_fft_scalar_cuFFT: env_cft_2xy_gpu ", " in fftxy ffty batch_2 istat = ", istat)
        end if
...
!        end do
        ...
        call env_fftx_error__(" env_fft_scalar_cuFFT: env_cft_2xy_gpu ", " in fftxy fftx istat = ", istat)
   ...
   CONTAINS
     ...
     SUBROUTINE env_init_plan()
       ...
       IF( cufft_plan_2d( icurrent) /= 0 ) THEN
           ...
           call env_fftx_error__(" env_fft_scalar_cuFFT: env_cft_2xy_gpu ", " cufftDestroy failed ", istat)
       ENDIF
       ...
       istat = cufftPlanMany( cufft_plan_2d( icurrent), RANK, FFT_DIM, &
                              ...
                              DATA_DIM, STRIDE, DIST, &
                              CUFFT_Z2Z, BATCH )
       call env_fftx_error__(" env_fft_scalar_cuFFT: env_cft_2xy_gpu ", " cufftPlanMany failed ", istat)
       ...
       IF( cufft_plan_x( icurrent) /= 0 ) THEN
           ...
           call env_fftx_error__(" env_fft_scalar_cuFFT: env_cft_2xy_gpu ", " cufftDestroy failed x ", istat)
       ENDIF
       IF( cufft_plan_y( 1, icurrent) /= 0 ) THEN
           istat = cufftDestroy( cufft_plan_y(1,icurrent) )
           call env_fftx_error__(" env_fft_scalar_cuFFT: env_cft_2xy_gpu ", " cufftDestroy failed y1 ", istat)
       ENDIF
       IF( cufft_plan_y( 2, icurrent) /= 0 ) THEN
           istat = cufftDestroy( cufft_plan_y(2,icurrent) )
           call env_fftx_error__(" env_fft_scalar_cuFFT: env_cft_2xy_gpu ", " cufftDestroy failed y2 ", istat)
       ENDIF
       ...
       istat = cufftPlanMany( cufft_plan_x( icurrent), RANK, FFT_DIM_X, &
                              ...
                              DATA_DIM_X, STRIDE_X, DIST_X, &
                              CUFFT_Z2Z, BATCH_X )
       call env_fftx_error__(" env_fft_scalar_cuFFT: env_cft_2xy_gpu ", " cufftPlanMany failed batch_x ", istat)

       istat = cufftPlanMany( cufft_plan_y( 1, icurrent), RANK, FFT_DIM_Y, &
                              DATA_DIM_Y, STRIDE_Y, DIST_Y, &
                              DATA_DIM_Y, STRIDE_Y, DIST_Y, &
                              CUFFT_Z2Z, BATCH_Y1 )
       call env_fftx_error__(" env_fft_scalar_cuFFT: env_cft_2xy_gpu ", " cufftPlanMany failed batch_y1 ", istat)

       istat = cufftPlanMany( cufft_plan_y( 2, icurrent), RANK, FFT_DIM_Y, &
                              DATA_DIM_Y, STRIDE_Y, DIST_Y, &
                              DATA_DIM_Y, STRIDE_Y, DIST_Y, &
                              CUFFT_Z2Z, BATCH_Y2 )
       call env_fftx_error__(" env_fft_scalar_cuFFT: env_cft_2xy_gpu ", " cufftPlanMany failed batch_y2 ", istat)
...
!     C_POINTER, save :: bw_plan(ndims) = 0
     ...
     IF ( nx < 1 ) &
         call env_fftx_error__('env_cfft3d',' nx is less than 1 ', 1)
     IF ( ny < 1 ) &
         call env_fftx_error__('env_cfft3d',' ny is less than 1 ', 1)
     IF ( nz < 1 ) &
         call env_fftx_error__('env_cfft3d',' nz is less than 1 ', 1)
     IF ( nx /= ldx .or. ny /= ldy .or. nz /= ldz ) &
         call env_fftx_error__('env_cfft3d',' leading dimensions must match data dimension ', 1)
     !
     !   Here initialize table only if necessary
     !
     CALL env_lookup()
     ...
     call env_fftx_error__(" env_fft_scalar_cuFFT: env_cfft3d_gpu ", " failed to set stream ", istat)

     IF( isign < 0 ) THEN

        istat = cufftExecZ2Z( cufft_plan_3d(ip), f_d(1), f_d(1), CUFFT_FORWARD )
        call env_fftx_error__(" env_fft_scalar_cuFFT: env_cfft3d_gpu ", " cufftExecZ2Z failed ", istat)
...
!$cuf kernel do(1) <<<*,(16,16,1),0,stream>>>
   ...
   CONTAINS
     ...
     SUBROUTINE env_init_plan()
       ...
       IF( cufft_plan_3d( icurrent) /= 0 ) THEN
           ...
           call env_fftx_error__(" env_fft_scalar_cuFFT: env_cfft3d_gpu ", " cufftDestroy failed ", istat)
       ENDIF
       ...
       istat = cufftPlanMany( cufft_plan_3d( icurrent), RANK, FFT_DIM, &
                              ...
                              DATA_DIM, STRIDE, DIST, &
                              CUFFT_Z2Z, BATCH )
       call env_fftx_error__(" env_fft_scalar_cuFFT: env_cfft3d_gpu ", " cufftPlanMany failed ", istat)

       !IF ( nx /= ldx .or. ny /= ldy .or. nz /= ldz ) &
       !  call env_fftx_error__('cfft3','not implemented',1)
   ...
   SUBROUTINE env_cfft3ds_gpu (f_d, nx, ny, nz, ldx, ldy, ldz, howmany, isign, &
     do_fft_z, do_fft_y, stream)
     !
     !     driver routine for 3d complex "reduced" fft - see env_cfft3d
     ...
     !     If not implemented, env_cfft3d is called instead
     !
     ...
     !
     ! env_cfft3d_gpu outperforms an explicit implementation of env_cfft3ds_gpu
     ! which is now buried in the commit history.
     !
     CALL env_cfft3d_gpu (f_d, nx, ny, nz, ldx, ldy, ldz, howmany, isign, stream)
     return
   END SUBROUTINE env_cfft3ds_gpu
#endif
!=----------------------------------------------------------------------=!
