...
!=----------------------------------------------------------------------=!
        ...
        PRIVATE
        !
        PUBLIC :: env_fft_type_descriptor
        PUBLIC :: env_fft_scatter_xy_gpu, env_fft_scatter_yz_gpu, env_fft_scatter_tg_gpu, &
                  env_fft_scatter_tg_opt_gpu, env_fft_scatter_many_yz_gpu
...
SUBROUTINE env_fft_scatter_xy_gpu ( desc, f_in_d, f_aux_d, nxx_, isgn, stream )
  ...
  USE cudafor
  !USE nvtx_fft
  USE env_fft_buffers, ONLY : env_check_buffers_size, f_in => pin_space_scatter_in, &
  ...
  CALL env_check_buffers_size(desc)
  !
  ierr = 0
  IF (isgn.gt.0) THEN
...
!
!-----------------------------------------------------------------------
SUBROUTINE env_fft_scatter_tg_gpu ( desc, f_in_d, f_aux_d, nxx_, isgn, stream )
  !-----------------------------------------------------------------------
  !
...
END SUBROUTINE env_fft_scatter_tg_gpu
!
!-----------------------------------------------------------------------
SUBROUTINE env_fft_scatter_tg_opt_gpu ( desc, f_in_d, f_out_d, nxx_, isgn, stream )
  !-----------------------------------------------------------------------
  !
  ! task group wavefunction redistribution
  !
...
END SUBROUTINE env_fft_scatter_tg_opt_gpu
...
SUBROUTINE env_fft_scatter_many_yz_gpu ( desc, f_in_d, f_aux_d, nxx_, isgn, howmany )
  ...
  CALL env_check_buffers_size(desc, howmany)
  desc_ismap_d => desc%ismap_d
  !
  ierr = 0
  IF (isgn.gt.0) THEN
