...
SUBROUTINE env_tg_gather( dffts, v, tg_v )
  ...
  TYPE(env_fft_type_descriptor), INTENT(in) :: dffts
...
#if defined(__MPI)
!used to be   CALL env_mp_sum(tg_v, dffts%comm2 )
  nsiz =dffts%nnr_tg
  CALL MPI_ALLREDUCE( MPI_IN_PLACE, tg_v, nsiz, MPI_DOUBLE_PRECISION, MPI_SUM, dffts%comm2, ierr )
  IF( ierr /= 0 ) CALL env_fftx_error__( ' env_tg_gather ', ' MPI_ALLREDUCE ', abs( ierr ) )
...
!  IF( ierr /= 0 ) CALL env_fftx_error__( ' env_tg_gather ', ' MPI_Allgatherv ', abs( ierr ) )
#endif
...
!----------------------------------------------------------------------------------------------------------------
!-real version 
SUBROUTINE env_tg_gather_gpu( dffts, v_d, tg_v_d )
  !
  USE env_fft_param
  USE env_fft_types,      ONLY : env_fft_type_descriptor
...
!  IF( ierr /= 0 ) CALL env_fftx_error__( ' env_tg_gather ', ' MPI_Allgatherv ', abs( ierr ) )
  ...
  !write (6,'(20f12.7)') (tg_v(i+dffts%nr1x*(i-1)), i=1,dffts%nr1x)
  RETURN
END SUBROUTINE env_tg_gather_gpu

!-complex version of previous routine
SUBROUTINE env_tg_cgather_gpu( dffts, v_d, tg_v_d )
  !
  USE env_fft_param
  USE env_fft_types,      ONLY : env_fft_type_descriptor
...
!  IF( ierr /= 0 ) CALL env_fftx_error__( ' env_tg_gather ', ' MPI_Allgatherv ', abs( ierr ) )
  ...
  !write (6,'(20f12.7)') (tg_v(i+dffts%nr1x*(i-1)), i=1,dffts%nr1x)
  RETURN
END SUBROUTINE env_tg_cgather_gpu
#endif
