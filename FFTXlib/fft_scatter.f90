...
!=----------------------------------------------------------------------=!
        ...
        PUBLIC :: env_fft_scatter_xy, env_fft_scatter_yz, env_fft_scatter_many_xy, env_fft_scatter_many_yz
        PUBLIC :: env_fft_scatter_tg, env_fft_scatter_tg_opt
...
!$omp end master
  ...
  if ( abs (isgn) == 1 ) then          ! It's a potential FFT
     CALL env_impl_xy( MAXVAL ( desc%nr2p ), desc%nproc2, desc%my_nr2p, desc%nr1p, desc%indp, desc%iplp, comm_)
  else if ( abs (isgn) == 2 ) then     ! It's a wavefunction FFT
     CALL env_impl_xy( MAXVAL ( desc%nr2p ), desc%nproc2, desc%my_nr2p, desc%nr1w, desc%indw, desc%iplw, comm_)
     ...
     CALL env_impl_xy( desc%nr2x, 1, desc%nr2x, nr1_temp, desc%indw_tg, desc%iplw, comm_)
  end if
  !
!$omp master
...
!$omp end master
  ...
  SUBROUTINE env_impl_xy(nr2px, nproc2, my_nr2p, nr1p_, indx, iplx, mpi_comm)
  IMPLICIT NONE
  !
...
!$omp end parallel do
     ...
     DO iproc2 = 1, nproc2
        ...
        !IF( abs(ierr) /= 0 ) CALL env_fftx_error__ ('env_fft_scatter', ' forward receive info<>0', abs(ierr) )
     ENDDO
...
!$omp end parallel do
     ...
     DO iproc2 = 1, nproc2
        CALL mpi_irecv( f_aux( (iproc2-1)*sendsize + 1 ), sendsize, &
                        MPI_DOUBLE_COMPLEX, iproc2-1, MPI_ANY_TAG, &
                        mpi_comm, rh(iproc2), ierr )
        ! IF( abs(ierr) /= 0 ) CALL env_fftx_error__ ('env_fft_scatter', ' backward receive info<>0', abs(ierr) )
     ENDDO
...
SUBROUTINE env_fft_scatter_tg ( desc, f_in, f_aux, nxx_, isgn )
  ...
  if ( isgn > 0 ) then
     ...
     IF( ierr /= 0 ) CALL env_fftx_error__( 'env_fft_scatter_tg', ' alltoall error 1 ', abs(ierr) )
     ...
     IF( ierr /= 0 ) CALL env_fftx_error__( 'env_fft_scatter_tg', ' alltoall error 2 ', abs(ierr) )
   end if
