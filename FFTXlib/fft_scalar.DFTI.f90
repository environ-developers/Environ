...
!$omp threadprivate(hand, dfti_first, zdims, icurrent, is_inplace)
     ...
     IF ( nsl <= 0 ) THEN
       ...
       IF ( nsl < 0 ) CALL env_fftx_error__(" env_fft_scalar: env_cft_1z ", " nsl out of range ", nsl)
     ...
     CALL env_start_clock( 'env_cft_1z' )
#endif
     ...
     IF (isign < 0) THEN
        ...
        ENDIF
        IF(dfti_status /= 0) &
           CALL env_fftx_error__(' env_cft_1z ',' stopped in DftiComputeForward '// DftiErrorMessage(dfti_status), dfti_status )
     ELSE IF (isign > 0) THEN
        ...
        ENDIF
        IF(dfti_status /= 0) &
           CALL env_fftx_error__(' env_cft_1z ',' stopped in DftiComputeBackward '// DftiErrorMessage(dfti_status), dfti_status )
     END IF

#if defined(__FFT_CLOCKS)
     CALL env_stop_clock( 'env_cft_1z' )
#endif
   ...
   CONTAINS !=------------------------------------------------=!
     ...
     SUBROUTINE env_init_dfti()
       ...
       IF(dfti_status /= 0)  &
         CALL env_fftx_error__(' env_cft_1z ',' stopped in DftiCreateDescriptor ', dfti_status )

       dfti_status = DftiSetValue(hand( icurrent )%desc, DFTI_NUMBER_OF_TRANSFORMS,nsl)
       IF(dfti_status /= 0) &
         CALL env_fftx_error__(' env_cft_1z ',' stopped in DFTI_NUMBER_OF_TRANSFORMS ', dfti_status )

       dfti_status = DftiSetValue(hand( icurrent )%desc,DFTI_INPUT_DISTANCE, ldz )
       IF(dfti_status /= 0) &
         CALL env_fftx_error__(' env_cft_1z ',' stopped in DFTI_INPUT_DISTANCE ', dfti_status )
       ...
       ENDIF
       IF(dfti_status /= 0) &
         CALL env_fftx_error__(' env_cft_1z ',' stopped in DFTI_PLACEMENT ', dfti_status )

       dfti_status = DftiSetValue(hand( icurrent )%desc,DFTI_OUTPUT_DISTANCE, ldz )
       IF(dfti_status /= 0) &
         CALL env_fftx_error__(' env_cft_1z ',' stopped in DFTI_OUTPUT_DISTANCE ', dfti_status )

       tscale = 1.0_DP/nz
       dfti_status = DftiSetValue( hand( icurrent )%desc, DFTI_FORWARD_SCALE, tscale);
       IF(dfti_status /= 0) &
         CALL env_fftx_error__(' env_cft_1z ',' stopped in DFTI_FORWARD_SCALE ', dfti_status )

       dfti_status = DftiSetValue( hand( icurrent )%desc, DFTI_BACKWARD_SCALE, DBLE(1) );
       IF(dfti_status /= 0) &
         CALL env_fftx_error__(' env_cft_1z ',' stopped in DFTI_BACKWARD_SCALE ', dfti_status )

       !dfti_status = DftiSetValue( hand( icurrent )%desc, DFTI_THREAD_LIMIT, 1 );
       !IF(dfti_status /= 0) &
       !  CALL env_fftx_error__(' env_cft_1z ',' stopped in DFTI_THREAD_LIMIT ', dfti_status )

       dfti_status = DftiCommitDescriptor(hand( icurrent )%desc)
       IF(dfti_status /= 0) &
         CALL env_fftx_error__(' env_cft_1z ',' stopped in DftiCommitDescriptor ', dfti_status )
...
#if defined(__FFT_CLOCKS)
     CALL env_start_clock( 'env_cft_2xy' )
#endif
...
#if defined(__FFT_CLOCKS)
     CALL env_stop_clock( 'env_cft_2xy' )
#endif
...
!
  ...
  !     parameters nx,ny,nz) are stored and re-used if available
   ...
   CONTAINS !=------------------------------------------------=!
     ...
     IF ( howmany < 1 ) &
         call env_fftx_error__('env_cfft3d',' howmany is less than 1 ', 1)
     END SUBROUTINE env_check_dims
     ...
     SUBROUTINE env_init_dfti()
      if( ASSOCIATED( hand(icurrent)%desc ) ) THEN
       ...
       IF(dfti_status /= 0)THEN
          WRITE(*,*) "stopped in env_cfft3d, DFTI_NUMBER_OF_TRANSFORMS", dfti_status
          STOP
       ENDIF
