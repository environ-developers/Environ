...
! ... module env_fft_param using iso_fortran_env from F2003.
! ... env_fftx_error_uniform__ must be called by all the the ranks in an MPI
...
! ... In cases that errors don't occur uniformly, env_fftx_error__ prints the
! ... error message and aborts the code. If it is used for uniform errors,
! ... repeated error messages may be printed.
! ... Use env_fftx_error_uniform__ wherever errors can be handled cleanly.
! ... env_fftx_error__ is the last choice you may consider.

SUBROUTINE env_fftx_error_uniform__( calling_routine, message, ierr, comm )
  !----------------------------------------------------------------------------
  !
  ...
  RETURN
  !
END SUBROUTINE env_fftx_error_uniform__
