...
!     parameters nz, nsl, ldz) are stored and re-used if available
     ...
     IF( .NOT. done ) THEN
       ...
       ! WRITE( stdout, fmt="('DEBUG env_cft_1z, reinitializing tables ', I3)" ) icurrent
...
!     parameters nx,ny,nzl,ldx) are stored and re-used if available
     ...
     IF( .NOT. done ) THEN
       ...
       ! WRITE( stdout, fmt="('DEBUG env_cft_2xy, reinitializing tables ', I3)" ) icurrent
...
!
  ...
  !     parameters nx,ny,nz) are stored and re-used if available
     ...
     IF ( howmany /= 1 ) &
         call env_fftx_error__('env_cfft3d',' howmany different from 1, not yetimplemented for ESSL ', 1)
     !
     !   Here initialize table only if necessary
     !
     ip = -1
...
SUBROUTINE env_cfft3ds (f, nx, ny, nz, ldx, ldy, ldz, howmany, isign, &
  ...
  ! WRITE( stdout, fmt="('DEBUG env_cfft3ds :',6I6)") nx, ny, nz, ldx, ldy, ldz
  ! WRITE( stdout, fmt="('DEBUG env_cfft3ds :',24I2)") do_fft_y
  ! WRITE( stdout, fmt="('DEBUG env_cfft3ds :',24I2)") do_fft_z


  IF( ny /= ldy ) &
    CALL env_fftx_error__(' env_cfft3ds ', ' wrong dimensions: ny /= ldy ', 1 )

     IF ( howmany /= 1 ) &
         call env_fftx_error__('env_cfft3ds',' howmany different from 1, not yetimplemented for ESSL ', 1)
