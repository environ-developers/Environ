...
!----------------------------------------------------------------------------
SUBROUTINE hessv_h_of_rho_r( rho, hessv )
  !----------------------------------------------------------------------------
  !
  ! ... Gradient of Hartree potential in R space from a total 
  !     (spinless) density in R space n(r)
  ...
  !
  REAL( DP ), INTENT(IN)     :: rho( dfftp%nnr )
  REAL( DP ), INTENT(OUT)    :: hessv( 3, 3, dfftp%nnr )
  !
  ! ... Local variables
  !
  COMPLEX( DP ), ALLOCATABLE :: rhoaux( : )
  ...
  DO ipol = 1, 3
     ...
     DO jpol = 1, 3
        ...
        DO ig = gstart, ngm
           ...
           gaux(dfftp%nl(ig)) = CMPLX(REAL(rhoaux(dfftp%nl(ig))),AIMAG(rhoaux(dfftp%nl(ig))),kind=dp) * fac 
           !
        END DO
        !
        ...
        if (do_comp_mt) then
           ...
           DO ig = gstart, ngm
              ...
              gaux(dfftp%nl(ig)) = gaux(dfftp%nl(ig)) + CMPLX(REAL(vaux(ig)),AIMAG(vaux(ig)),kind=dp)*fac 
           END DO
           DEALLOCATE( rgtot, vaux )
        end if
        !
...
SUBROUTINE field_of_gradrho( gradrho, e )
  ...
  ! ... Declares variables
  !
  REAL( DP ), INTENT(IN)     :: gradrho( 3, dfftp%nnr )
  REAL( DP ), INTENT(OUT)    :: e( dfftp%nnr )
  !
  ! ... Local variables
  !
  COMPLEX( DP ), ALLOCATABLE :: aux( : )
  ...
  DO ipol = 1, 3
     ...
     DO ig = gstart, ngm
        !
        fac = g(ipol,ig) / gg(ig)
        aux(dfftp%nl(ig)) = CMPLX(-AIMAG(gaux(dfftp%nl(ig))),REAL(gaux(dfftp%nl(ig))),kind=dp) * fac
        !
     END DO
     !
     ! ...and add the factor e2*fpi/2\pi/a coming from the missing prefactor of
     ...
     IF ( do_comp_mt ) THEN
        ...
        DO ig = gstart, ngm
           !
           fac = g(ipol,ig) * tpiba
           aux(dfftp%nl(ig)) = aux(dfftp%nl(ig)) + CMPLX(-AIMAG(vaux(ig)),REAL(vaux(ig)),kind=dp)*fac
           !
        END DO
        !
     END IF
  ...
  IF ( gamma_only ) THEN
     !
     eaux(dfftp%nlm(:)) = &
          CMPLX( REAL( eaux(dfftp%nl(:)) ), -AIMAG( eaux(dfftp%nl(:)) ) ,kind=DP)
     !
  END IF
  !
  ! ... bring back to R-space, (\grad_ipol a)(r) ...
  !
  CALL invfft ('Rho', eaux, dfftp)
  !
  e(:) = REAL( eaux(:) )
  !
  DEALLOCATE(eaux)
