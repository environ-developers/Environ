...
!     env_init_clocks( go )    initialization - must be called first
...
!     env_get_clock( label )     return wall time measured by clock "label"
...
!#define __TRACE
! ... See also comments in subroutine env_print_this_clock about parallel case
!
!----------------------------------------------------------------------------
MODULE env_mytime
  !----------------------------------------------------------------------------
...
SUBROUTINE env_init_clocks( go )
  ...
  ! ... GPU related timers
  USE env_mytime, ONLY : gpu_starts, gpu_stops, gpu_called, gputime
#if defined (__TRACE)
  ...
  IF (ierr/=0) then
     WRITE( stdout, fmt='( "*** error in env_init_clocks call to mpi_comm_rank ***")' )
...
!
!----------------------------------------------------------------------------
SUBROUTINE env_set_trace_max_depth(max_depth_)
  USE env_mytime, ONLY: max_print_depth
  IMPLICIT NONE
  INTEGER  :: max_depth_
  ! 
  max_print_depth = max_depth_   
END SUBROUTINE env_set_trace_max_depth 
! 
!----------------------------------------------------------------------------
SUBROUTINE env_start_clock( label )
  ...
  WRITE( stdout,'(I3," depth=",I2," env_start_clock ",A )') mpime,trace_depth, label ; FLUSH(stdout)
...
!                           & " already started")' ) n, label_
        ELSE
           ...
           call env_nvtxStartRange(label_, n)
        ENDIF
        !
        RETURN
        !
...
SUBROUTINE env_start_clock_gpu( label )
  ...
  USE env_nvtx,      ONLY : env_nvtxStartRange
#if defined(__CUDA)
  USE cudafor
#endif
  !
...
SUBROUTINE env_stop_clock( label )
  ...
  trace_depth = trace_depth - 1
  if (trace_depth <= max_print_depth ) &  ! used to gauge the ammount of output
  WRITE( stdout,'(I3," depth=",I2," env_stop_clock ",A )') mpime, trace_depth, label ; FLUSH(stdout)
  ...
  DO n = 1, nclock
     ...
     IF ( clock_label(n) == label_ ) THEN
        ...
        IF ( t0cpu(n) == notrunning ) THEN
           !
           WRITE( stdout, '("env_stop_clock: clock # ",I2," for ",A12, " not running")' ) n, label
           !
        ELSE
           !
...
SUBROUTINE env_stop_clock_gpu( label )
  ...
  USE env_nvtx,      ONLY:  env_nvtxEndRange
#if defined(__CUDA)
  USE cudafor
#endif
  !
...
SUBROUTINE env_print_clock( label )
  ...
  USE env_mytime,     ONLY : nclock, clock_label, gpu_called
  !
  IMPLICIT NONE
  !
  CHARACTER(len=*) :: label
...
!#define PRINT_AVG_CPU_TIME_PER_THREAD
  ...
  !
  ! CALL env_mp_max( elapsed_cpu_time, intra_image_comm )
  ! CALL env_mp_max( elapsed_wall_time, intra_image_comm )
  ! CALL env_mp_max( nmax, intra_image_comm )
  !
...
SUBROUTINE env_print_this_clock_gpu( n )
  ...
  !
  USE env_util_param, ONLY : DP, stdout
  USE env_mytime,     ONLY : clock_label, gputime, called, gpu_called, mpi_per_thread                
  !
  IMPLICIT NONE
  !
  INTEGER  :: n
...
FUNCTION env_get_clock( label )
  ...
  DO n = 1, nclock
     ...
     IF ( label == clock_label(n) ) THEN
        ...
        ENDIF
        !
        ! ... See comments in subroutine env_print_this_clock about parallel case
        !
        ! CALL env_mp_max( env_get_clock, intra_image_comm )
        !
        RETURN
        !
     ENDIF
