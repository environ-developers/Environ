...
! depends on the __MSGSIZ_MAX definition
   ...
   SUBROUTINE env_deallocate_buffers_gpu
       ...
       IMPLICIT NONE
       !
       IF (ALLOCATED (mp_buff_r_d) .AND. ALLOCATED (mp_buff_i_d)) DEALLOCATE(mp_buff_r_d, mp_buff_i_d)
       !
...
2       CONTINUE
   ...
   SUBROUTINE env_bcast_integer_gpu( array_d, n, root, gid )
...
#if defined __TRACE
        write(*,*) 'env_bcast_integer_gpu IN'
#endif
        ...
        IF( n <= msgsiz_max ) THEN
           ...
           IF( ierr /= 0 ) CALL env_errore( ' env_bcast_integer_gpu ', ' error in mpi_bcast 1 ', ierr )
        ELSE
           nblk   = n / msgsiz_max
           blksiz = msgsiz_max
           DO iblk = 1, nblk
              ...
              IF( ierr /= 0 ) CALL env_errore( ' env_bcast_integer_gpu ', ' error in mpi_bcast 2 ', ierr )
           END DO
           blksiz = MOD( n, msgsiz_max )
           IF( blksiz > 0 ) THEN
              istart = nblk * msgsiz_max + 1
              CALL MPI_BCAST( array_d( istart ), blksiz, MPI_INTEGER, root, gid, ierr )
              IF( ierr /= 0 ) CALL env_errore( ' env_bcast_integer_gpu ', ' error in mpi_bcast 3 ', ierr )
           END IF
        END IF
        GO TO 2 ! Skip sync, already done by MPI call 
1       CONTINUE
...
2       CONTINUE
#if defined __TRACE
        write(*,*) 'env_bcast_integer_gpu OUT'
#endif
#endif
        RETURN
   ...
   SUBROUTINE env_bcast_logical_gpu( array_d, n, root, gid )
...
#if defined __TRACE
        write(*,*) 'env_bcast_logical_gpu IN'
#endif
        ...
        IF( n <= msgsiz_max ) THEN
           ...
           IF( ierr /= 0 ) CALL env_errore( ' env_bcast_logical_gpu ', ' error in mpi_bcast 1 ', ierr )
        ELSE
           nblk   = n / msgsiz_max
           blksiz = msgsiz_max
           DO iblk = 1, nblk
              ...
              IF( ierr /= 0 ) CALL env_errore( ' env_bcast_logical_gpu ', ' error in mpi_bcast 2 ', ierr )
           END DO
           blksiz = MOD( n, msgsiz_max )
           IF( blksiz > 0 ) THEN
              istart = nblk * msgsiz_max + 1
              CALL MPI_BCAST( array_d( istart ), blksiz, MPI_LOGICAL, root, gid, ierr )
              IF( ierr /= 0 ) CALL env_errore( ' env_bcast_logical_gpu ', ' error in mpi_bcast 3 ', ierr )
           END IF
        END IF
...
2       CONTINUE
#if defined __TRACE
        write(*,*) 'env_bcast_logical_gpu OUT'
#endif
#endif
        RETURN
...
SUBROUTINE env_reduce_base_real_gpu( dim, ps_d, comm, root )
  ...
  !
#if defined __TRACE
  write(*,*) 'env_reduce_base_real_gpu IN'
#endif
  !
  ...
  IF( root >= 0 ) THEN
     ...
     IF( info /= 0 ) CALL env_errore( 'env_reduce_base_real_gpu', 'error in mpi_reduce 1', info )
  ELSE
     CALL MPI_ALLREDUCE( MPI_IN_PLACE, ps_d, dim, MPI_DOUBLE_PRECISION, MPI_SUM, comm, info )
     IF( info /= 0 ) CALL env_errore( 'env_reduce_base_real_gpu', 'error in mpi_allreduce 1', info )
  END IF
  !
  GO TO 2 ! Skip sync, already done by MPI call 
1 CONTINUE
...
2 CONTINUE
  !
#if defined __TRACE
  write(*,*) 'env_reduce_base_real_gpu OUT'
#endif
  !
#endif
  !
...
SUBROUTINE env_reduce_base_real_gpu( dim, ps_d, comm, root )
  ...
  !
  USE env_util_param, ONLY : DP
  USE env_data_buffer,    ONLY : mp_buff_r_d
  USE env_parallel_include
  USE cudafor
  !
  IMPLICIT NONE
  ...
  IF( info /= 0 ) CALL env_errore( 'env_reduce_base_real_gpu', 'error in mpi_comm_size', info )

  CALL mpi_comm_rank( comm, myid, info )
  IF( info /= 0 ) CALL env_errore( 'env_reduce_base_real_gpu', 'error in mpi_comm_rank', info )
  !
  ...
  DO n = 1, nbuf
     ...
     IF( root < 0 ) THEN
        ...
        IF( info /= 0 ) CALL env_errore( 'env_reduce_base_real_gpu', 'error in cudaMemcpy ', info )
...
SUBROUTINE env_reduce_base_integer_gpu( dim, ps_d, comm, root )
  ...
  !
#if defined __TRACE
  write(*,*) 'env_reduce_base_integer_gpu IN'
#endif
  !
  IF ( dim <= 0 ) GO TO 1  ! go to the end of the subroutine
  !
  ...
  IF( root >= 0 ) THEN
     ...
     IF( info /= 0 ) CALL env_errore( 'env_reduce_base_integer_gpu', 'error in mpi_reduce 1', info )
  ELSE
     CALL MPI_ALLREDUCE( MPI_IN_PLACE, ps_d, dim, MPI_INTEGER, MPI_SUM, comm, info )
     IF( info /= 0 ) CALL env_errore( 'env_reduce_base_integer_gpu', 'error in mpi_allreduce 1', info )
  END IF
  !
  GO TO 2 ! Skip sync, already done by MPI call 
1 CONTINUE
...
2 CONTINUE
  !
#if defined __TRACE
  write(*,*) 'env_reduce_base_integer_gpu OUT'
#endif
  !
#endif
  !
...
SUBROUTINE env_reduce_base_integer_gpu( dim, ps_d, comm, root )
  ...
  CALL mpi_comm_size( comm, nproc, info )
  IF( info /= 0 ) CALL env_errore( 'env_reduce_base_integer_gpu', 'error in mpi_comm_size', info )

  CALL mpi_comm_rank( comm, myid, info )
  IF( info /= 0 ) CALL env_errore( 'env_reduce_base_integer_gpu', 'error in mpi_comm_rank', info )
  !
  IF ( dim <= 0 .OR. nproc <= 1 ) GO TO 1  ! go to the end of the subroutine
  !
  ! ... synchronize processes
  ...
  DO n = 1, nbuf
     ...
     IF( root < 0 ) THEN
        ...
        !ps((1+(n-1)*maxb):(n*maxb)) = mp_buff_r(1:maxb)
        IF( info /= 0 ) CALL env_errore( 'env_reduce_base_integer_gpu', 'error in cudaMemcpy ', info )
     ELSE IF( root == myid ) THEN
        info = cudaMemcpy( ps_d((1+(n-1)*maxb)), mp_buff_i_d(1), maxb, cudaMemcpyDeviceToDevice )
        !ps((1+(n-1)*maxb):(n*maxb)) = mp_buff_r(1:maxb)
        IF( info /= 0 ) CALL env_errore( 'env_reduce_base_integer_gpu', 'error in cudaMemcpy ', info )
  ...
  IF ( ( dim - nbuf * maxb ) > 0 ) THEN
     ...
     IF( root >= 0 ) THEN
        ...
        IF( info /= 0 ) CALL env_errore( 'env_reduce_base_integer_gpu', 'error in mpi_reduce 2', info )
     ELSE
        CALL MPI_ALLREDUCE( ps_d(1+nbuf*maxb), mp_buff_i_d, (dim-nbuf*maxb), MPI_INTEGER, MPI_SUM, comm, info )
        IF( info /= 0 ) CALL env_errore( 'env_reduce_base_integer_gpu', 'error in mpi_allreduce 2', info )
     END IF
     !
     IF( root < 0 ) THEN
...
SUBROUTINE env_reduce_base_real_to_gpu( dim, ps_d, psout_d, comm, root )
  ...
  !
#if defined __TRACE
  write(*,*) 'env_reduce_base_real_to_gpu IN'
#endif

  CALL mpi_comm_size( comm, nproc, info )
  IF( info /= 0 ) CALL env_errore( 'env_reduce_base_real_to_gpu', 'error in mpi_comm_size', info )

  CALL mpi_comm_rank( comm, myid, info )
  IF( info /= 0 ) CALL env_errore( 'env_reduce_base_real_to_gpu', 'error in mpi_comm_rank', info )
  !
  ...
  DO n = 1, nbuf
     ...
     IF( root >= 0 ) THEN
        ...
        IF( info /= 0 ) CALL env_errore( 'env_reduce_base_real_to_gpu', 'error in mpi_reduce 1', info )
     ELSE
        CALL MPI_ALLREDUCE( ps_d(1+(n-1)*maxb), psout_d(1+(n-1)*maxb), maxb, MPI_DOUBLE_PRECISION, MPI_SUM, comm, info )
        IF( info /= 0 ) CALL env_errore( 'env_reduce_base_real_to_gpu', 'error in mpi_allreduce 1', info )
     END IF
     !                    
  END DO
  !
  ...
  IF ( ( dim - nbuf * maxb ) > 0 ) THEN
     ...
     IF( root >= 0 ) THEN
        ...
        IF( info /= 0 ) CALL env_errore( 'env_reduce_base_real_to_gpu', 'error in mpi_reduce 2', info )
     ELSE
        CALL MPI_ALLREDUCE( ps_d(1+nbuf*maxb), psout_d(1+nbuf*maxb), (dim-nbuf*maxb), MPI_DOUBLE_PRECISION, MPI_SUM, comm, info )
        IF( info /= 0 ) CALL env_errore( 'env_reduce_base_real_to_gpu', 'error in mpi_allreduce 2', info )
     END IF
     !
  END IF
  !
...
2 CONTINUE
  !
#if defined __TRACE
  write(*,*) 'env_reduce_base_real_to_gpu OUT'
#endif
  !
#endif
  !
...
SUBROUTINE env_reduce_base_integer_to_gpu( dim, ps_d, psout_d, comm, root )
  ...
  !
#if defined __TRACE
  write(*,*) 'env_reduce_base_integer_to_gpu IN'
#endif

  CALL mpi_comm_size( comm, nproc, info )
  IF( info /= 0 ) CALL env_errore( 'env_reduce_base_integer_to_gpu', 'error in mpi_comm_size', info )

  CALL mpi_comm_rank( comm, myid, info )
  IF( info /= 0 ) CALL env_errore( 'env_reduce_base_integer_to_gpu', 'error in mpi_comm_rank', info )
  !
  IF ( dim > 0 .AND. nproc <= 1 ) THEN
     psout_d = ps_d
  END IF
  ...
  DO n = 1, nbuf
     ...
     IF( root >= 0 ) THEN
        ...
        IF( info /= 0 ) CALL env_errore( 'env_reduce_base_integer_to_gpu', 'error in mpi_reduce 1', info )
     ELSE
        CALL MPI_ALLREDUCE( ps_d(1+(n-1)*maxb), psout_d( 1+(n-1)*maxb ), maxb, MPI_INTEGER, MPI_SUM, comm, info )
        IF( info /= 0 ) CALL env_errore( 'env_reduce_base_integer_to_gpu', 'error in mpi_allreduce 1', info )
     END IF
     !                    
  END DO
  !
  ...
  IF ( ( dim - nbuf * maxb ) > 0 ) THEN
     ...
     IF( root >= 0 ) THEN
        ...
        IF( info /= 0 ) CALL env_errore( 'env_reduce_base_integer_to_gpu', 'error in mpi_reduce 2', info )
     ELSE
        CALL MPI_ALLREDUCE( ps_d(1+nbuf*maxb), psout_d(1+nbuf*maxb), (dim-nbuf*maxb), MPI_INTEGER, MPI_SUM, comm, info )
        IF( info /= 0 ) CALL env_errore( 'env_reduce_base_integer_to_gpu', 'error in mpi_allreduce 2', info )
     END IF
     !
  END IF
  !
...
2 CONTINUE
  !
#if defined __TRACE
  write(*,*) 'env_reduce_base_integer_to_gpu OUT'
#endif
  !
#endif
  !
...
SUBROUTINE env_parallel_min_integer_gpu( dim, ps_d, comm, root )
  ...
  !
#if defined __TRACE
  write(*,*) 'env_parallel_min_integer_gpu IN'
#endif
  !
  CALL mpi_comm_size( comm, nproc, info )
  IF( info /= 0 ) CALL env_errore( 'env_parallel_min_integer_gpu', 'error in mpi_comm_size', info )

  CALL mpi_comm_rank( comm, myid, info )
  IF( info /= 0 ) CALL env_errore( 'env_parallel_min_integer_gpu', 'error in mpi_comm_rank', info )
  !
  IF ( dim <= 0 .OR. nproc <= 1 ) GO TO 1 ! go to the sync and later to the end of the subroutine
  !
  ! ... synchronize processes
  ...
  DO n = 1, nbuf
     ...
     IF( root >= 0 ) THEN
        ...
        IF( info /= 0 ) CALL env_errore( 'env_parallel_min_integer_gpu', 'error in mpi_reduce 1', info )
     ELSE
        CALL MPI_ALLREDUCE( ps_d(1+(n-1)*maxb), buff, maxb, MPI_INTEGER, MPI_MIN, comm, info )
        IF( info /= 0 ) CALL env_errore( 'env_parallel_min_integer_gpu', 'error in mpi_allreduce 1', info )
     END IF
     !
     IF( root < 0 ) THEN
        !ps_d((1+(n-1)*maxb):(n*maxb)) = buff(1:maxb)
        info = cudaMemcpy( ps_d(1+(n-1)*maxb), buff(1), maxb, cudaMemcpyDeviceToDevice )
        IF( info /= 0 ) CALL env_errore( 'env_parallel_min_integer_gpu', 'error in cudaMemcpy ', info )
     ELSE IF( root == myid ) THEN
        !ps_d((1+(n-1)*maxb):(n*maxb)) = buff(1:maxb)
        info = cudaMemcpy( ps_d((1+(n-1)*maxb)), buff(1), maxb, cudaMemcpyDeviceToDevice )
        IF( info /= 0 ) CALL env_errore( 'env_parallel_min_integer_gpu', 'error in cudaMemcpy ', info )
  ...
  IF ( ( dim - nbuf * maxb ) > 0 ) THEN
     ...
     IF( root >= 0 ) THEN
        ...
        IF( info /= 0 ) CALL env_errore( 'env_parallel_min_integer_gpu', 'error in mpi_reduce 2', info )
     ELSE
        CALL MPI_ALLREDUCE( ps_d(1+nbuf*maxb), buff, (dim-nbuf*maxb), MPI_INTEGER, MPI_MIN, comm, info )
        IF( info /= 0 ) CALL env_errore( 'env_parallel_min_integer_gpu', 'error in mpi_allreduce 2', info )
     END IF
     !
     IF( root < 0 ) THEN
...
2 CONTINUE
  !
#if defined __TRACE
  write(*,*) 'env_parallel_min_integer_gpu OUT'
#endif
  !
#endif
  !
...
SUBROUTINE env_parallel_max_integer_gpu( dim, ps_d, comm, root )
  ...
  !
#if defined __TRACE
  write(*,*) 'env_parallel_max_integer_gpu IN'
#endif
  CALL mpi_comm_size( comm, nproc, info )
  IF( info /= 0 ) CALL env_errore( 'env_parallel_max_integer_gpu', 'error in mpi_comm_size', info )

  CALL mpi_comm_rank( comm, myid, info )
  IF( info /= 0 ) CALL env_errore( 'env_parallel_max_integer_gpu', 'error in mpi_comm_rank', info )
  !
  IF ( dim <= 0 .OR. nproc <= 1 ) GO TO 1 ! go to the sync and later to the end of the subroutine
  !
  ! ... synchronize processes
  ...
  DO n = 1, nbuf
     ...
     IF( root >= 0 ) THEN
        ...
        IF( info /= 0 ) CALL env_errore( 'env_parallel_max_integer_gpu', 'error in mpi_reduce 1', info )
     ELSE
        CALL MPI_ALLREDUCE( ps_d(1+(n-1)*maxb), buff, maxb, MPI_INTEGER, MPI_MAX, comm, info )
        IF( info /= 0 ) CALL env_errore( 'env_parallel_max_integer_gpu', 'error in mpi_allreduce 1', info )
     END IF
     !
     IF( root < 0 ) THEN
        !ps_d((1+(n-1)*maxb):(n*maxb)) = buff(1:maxb)
        info = cudaMemcpy( ps_d(1+(n-1)*maxb), buff(1), maxb, cudaMemcpyDeviceToDevice )
        IF( info /= 0 ) CALL env_errore( 'env_parallel_max_integer_gpu', 'error in cudaMemcpy ', info )
     ELSE IF( root == myid ) THEN
        !ps_d((1+(n-1)*maxb):(n*maxb)) = buff(1:maxb)
        info = cudaMemcpy( ps_d((1+(n-1)*maxb)), buff(1), maxb, cudaMemcpyDeviceToDevice )
        IF( info /= 0 ) CALL env_errore( 'env_parallel_max_integer_gpu', 'error in cudaMemcpy ', info )
  ...
  IF ( ( dim - nbuf * maxb ) > 0 ) THEN
     ...
     IF( root >= 0 ) THEN
        ...
        IF( info /= 0 ) CALL env_errore( 'env_parallel_max_integer_gpu', 'error in mpi_reduce 2', info )
     ELSE
        CALL MPI_ALLREDUCE( ps_d(1+nbuf*maxb), buff, (dim-nbuf*maxb), MPI_INTEGER, MPI_MAX, comm, info )
        IF( info /= 0 ) CALL env_errore( 'env_parallel_max_integer_gpu', 'error in mpi_allreduce 2', info )
     END IF
     !
     IF( root < 0 ) THEN
        info = cudaMemcpy( ps_d((1+nbuf*maxb)), buff(1), dim-nbuf*maxb, cudaMemcpyDeviceToDevice )
...
2 CONTINUE
  !
#if defined __TRACE
  write(*,*) 'env_parallel_max_integer_gpu OUT'
#endif
#endif
  !
  RETURN
...
SUBROUTINE env_parallel_min_real_gpu( dim, ps_d, comm, root )
  ...
  !
#if defined __TRACE
  write(*,*) 'env_parallel_min_real_gpu IN'
#endif
  CALL mpi_comm_size( comm, nproc, info )
  IF( info /= 0 ) CALL env_errore( 'env_parallel_min_real_gpu', 'error in mpi_comm_size', info )

  CALL mpi_comm_rank( comm, myid, info )
  IF( info /= 0 ) CALL env_errore( 'env_parallel_min_real_gpu', 'error in mpi_comm_rank', info )
  !
  IF ( dim <= 0 .OR. nproc <= 1 ) GO TO 1 ! go to the sync and later to the end of the subroutine
  !
  ! ... synchronize processes
  ...
  DO n = 1, nbuf
     ...
     IF( root >= 0 ) THEN
        ...
        IF( info /= 0 ) CALL env_errore( 'env_parallel_min_real_gpu', 'error in mpi_reduce 1', info )
     ELSE
        CALL MPI_ALLREDUCE( ps_d(1+(n-1)*maxb), buff, maxb, MPI_DOUBLE_PRECISION, MPI_MIN, comm, info )
        IF( info /= 0 ) CALL env_errore( 'env_parallel_min_real_gpu', 'error in mpi_allreduce 1', info )
     END IF
     !
     IF( root < 0 ) THEN
        !ps_d((1+(n-1)*maxb):(n*maxb)) = buff(1:maxb)
  ...
  IF ( ( dim - nbuf * maxb ) > 0 ) THEN
     ...
     IF( root >= 0 ) THEN
        ...
        IF( info /= 0 ) CALL env_errore( 'env_parallel_min_real_gpu', 'error in mpi_reduce 2', info )
     ELSE
        CALL MPI_ALLREDUCE( ps_d(1+nbuf*maxb), buff, (dim-nbuf*maxb), MPI_DOUBLE_PRECISION, MPI_MIN, comm, info )
        IF( info /= 0 ) CALL env_errore( 'env_parallel_min_real_gpu', 'error in mpi_allreduce 2', info )
     END IF
     !
     IF( root < 0 ) THEN
        info = cudaMemcpy( ps_d((1+nbuf*maxb)), buff(1), dim-nbuf*maxb, cudaMemcpyDeviceToDevice )
...
2 CONTINUE
  !
#if defined __TRACE
  write(*,*) 'env_parallel_min_real_gpu OUT'
#endif
#endif
  !
  RETURN
...
SUBROUTINE env_parallel_max_real_gpu( dim, ps_d, comm, root )
  ...
  !
#if defined __TRACE
  write(*,*) 'env_parallel_max_real_gpu IN'
#endif
  !
  CALL mpi_comm_size( comm, nproc, info )
  IF( info /= 0 ) CALL env_errore( 'env_parallel_max_real_gpu', 'error in mpi_comm_size', info )

  CALL mpi_comm_rank( comm, myid, info )
  IF( info /= 0 ) CALL env_errore( 'env_parallel_max_real_gpu', 'error in mpi_comm_rank', info )
  !
  ...
  DO n = 1, nbuf
     ...
     IF( root >= 0 ) THEN
        ...
        IF( info /= 0 ) CALL env_errore( 'env_parallel_max_real_gpu', 'error in mpi_reduce 1', info )
     ELSE
        CALL MPI_ALLREDUCE( ps_d(1+(n-1)*maxb), buff, maxb, MPI_DOUBLE_PRECISION, MPI_max, comm, info )
        IF( info /= 0 ) CALL env_errore( 'env_parallel_max_real_gpu', 'error in mpi_allreduce 1', info )
     END IF
     !
     IF( root < 0 ) THEN
        !ps_d((1+(n-1)*maxb):(n*maxb)) = buff(1:maxb)
        info = cudaMemcpy( ps_d(1+(n-1)*maxb), buff(1), maxb, cudaMemcpyDeviceToDevice )
        IF( info /= 0 ) CALL env_errore( 'env_parallel_max_real_gpu', 'error in cudaMemcpy ', info )
     ELSE IF( root == myid ) THEN
        !ps_d((1+(n-1)*maxb):(n*maxb)) = buff(1:maxb)
        info = cudaMemcpy( ps_d((1+(n-1)*maxb)), buff(1), maxb, cudaMemcpyDeviceToDevice )
        IF( info /= 0 ) CALL env_errore( 'env_parallel_max_real_gpu', 'error in cudaMemcpy ', info )
  ...
  IF ( ( dim - nbuf * maxb ) > 0 ) THEN
     ...
     IF( root >= 0 ) THEN
        ...
        IF( info /= 0 ) CALL env_errore( 'env_parallel_max_real_gpu', 'error in mpi_reduce 2', info )
     ELSE
        CALL MPI_ALLREDUCE( ps_d(1+nbuf*maxb), buff, (dim-nbuf*maxb), MPI_DOUBLE_PRECISION, MPI_max, comm, info )
        IF( info /= 0 ) CALL env_errore( 'env_parallel_max_real_gpu', 'error in mpi_allreduce 2', info )
     END IF
     !
     IF( root < 0 ) THEN
        info = cudaMemcpy( ps_d((1+nbuf*maxb)), buff(1), dim-nbuf*maxb, cudaMemcpyDeviceToDevice )
...
2 CONTINUE
  !
#if defined __TRACE
  write(*,*) 'env_parallel_max_real_gpu OUT'
#endif
  !
#endif
  !
