...
MODULE env_mp
!------------------------------------------------------------------------------!
  USE env_util_param,     ONLY : DP, stdout, i8b
  ...
  PUBLIC :: env_mp_start, env_mp_abort, env_mp_stop, env_mp_end, &
    ...
    env_mp_circular_shift_left, env_mp_circular_shift_left_start, &
    env_mp_get_comm_null, env_mp_get_comm_self, env_mp_count_nodes, &
    env_mp_type_create_column_section, env_mp_type_create_row_section, env_mp_type_free, &
    env_mp_allgather, env_mp_waitall, env_mp_testall
  !
  INTERFACE env_mp_bcast
    MODULE PROCEDURE env_mp_bcast_i1, env_mp_bcast_r1, env_mp_bcast_c1, &
      env_mp_bcast_z, env_mp_bcast_zv, &
      env_mp_bcast_iv, env_mp_bcast_i8v, env_mp_bcast_rv, env_mp_bcast_cv, env_mp_bcast_l, env_mp_bcast_rm, &
  ...
  INTERFACE env_mp_sum
    MODULE PROCEDURE env_mp_sum_i1, env_mp_sum_iv, env_mp_sum_i8v, env_mp_sum_im, env_mp_sum_it, env_mp_sum_i4, env_mp_sum_i5, &
  ...
  INTERFACE env_mp_get
    MODULE PROCEDURE env_mp_get_r1, env_mp_get_rv, env_mp_get_cv, env_mp_get_i1, env_mp_get_iv, env_mp_get_rm, env_mp_get_cm
#if defined(__CUDA)
   ...
   INTERFACE env_mp_allgather
     ...
     MODULE PROCEDURE env_mp_allgatherv_inplace_real_array
#if defined(__CUDA)
     MODULE PROCEDURE env_mp_allgatherv_inplace_cplx_array_gpu
     MODULE PROCEDURE env_mp_allgatherv_inplace_real_array_gpu
#endif
   END INTERFACE
   ! 
   ...
   END INTERFACE
   ! 
   INTERFACE env_mp_circular_shift_left_start
     MODULE PROCEDURE env_mp_circular_shift_left_start_i0, &
       env_mp_circular_shift_left_start_i1, &
       env_mp_circular_shift_left_start_i2, &
       env_mp_circular_shift_left_start_r2d, &
       env_mp_circular_shift_left_start_c2d
   END INTERFACE
   ! 
   INTERFACE env_mp_type_create_column_section
     MODULE PROCEDURE env_mp_type_create_cplx_column_section
     MODULE PROCEDURE env_mp_type_create_real_column_section
#if defined(__CUDA)
     MODULE PROCEDURE env_mp_type_create_cplx_column_section_gpu
     MODULE PROCEDURE env_mp_type_create_real_column_section_gpu
#endif
   END INTERFACE

   INTERFACE env_mp_type_create_row_section
     MODULE PROCEDURE env_mp_type_create_cplx_row_section
     MODULE PROCEDURE env_mp_type_create_real_row_section
#if defined(__CUDA)
     MODULE PROCEDURE env_mp_type_create_cplx_row_section_gpu
     MODULE PROCEDURE env_mp_type_create_real_row_section_gpu
#endif
   END INTERFACE
!------------------------------------------------------------------------------!
!
...
! waits till all request are completed
      SUBROUTINE env_mp_waitall(requests)
! ...
        IMPLICIT NONE
...
#endif
        RETURN
      END SUBROUTINE env_mp_waitall

!tests all requests
      SUBROUTINE env_mp_testall(requests, flag)
      ! ...
         IMPLICIT NONE
          INTEGER, INTENT (INOUT) :: requests(:)
          INTEGER :: ierr
...
!..env_mp_bcast
      ...
      ! 
      !------------------------------------------------------------------------------!
      SUBROUTINE env_mp_bcast_i8v(msg, source, gid)
      !------------------------------------------------------------------------------!
      !! 
      ...
      INTEGER :: msglen
      msglen = SIZE(msg)
      CALL env_bcast_integer8(msg, msglen, source, gid)
#endif
      !------------------------------------------------------------------------------!
      END SUBROUTINE env_mp_bcast_i8v
      !------------------------------------------------------------------------------!
      !
      !------------------------------------------------------------------------------!
...
!
      SUBROUTINE env_mp_stop(code)
        ...
        INTEGER :: ierr
        WRITE( stdout, fmt='( "*** error in Message Passing (env_mp) module ***")' )
...
!..env_mp_sum
      ...
      ! 
      !------------------------------------------------------------------------------!
      SUBROUTINE env_mp_sum_i8v(msg, gid)
      !------------------------------------------------------------------------------!
      !! 
      !! MPI sum an integer vector from all cores and bcast the result to all.  
      !! 
      ...
      INTEGER :: msglen
      msglen = SIZE(msg)
      CALL env_reduce_base_integer8(msglen, msg, gid, -1)
#endif
      !------------------------------------------------------------------------------!
      END SUBROUTINE env_mp_sum_i8v
      !------------------------------------------------------------------------------!
      !
      !------------------------------------------------------------------------------!
...
!------------------------------------------------------------------------------!
      ...
      SUBROUTINE env_mp_sum_i4(msg,gid)
        IMPLICIT NONE
        INTEGER, INTENT (INOUT) :: msg(:,:,:,:)
        INTEGER, INTENT (IN) :: gid
#if defined(__MPI)
        ...
        CALL env_reduce_base_integer( msglen, msg, gid, -1 )
#endif
      END SUBROUTINE env_mp_sum_i4

!------------------------------------------------------------------------------!

      SUBROUTINE env_mp_sum_i5(msg,gid)
        IMPLICIT NONE
        INTEGER, INTENT (INOUT) :: msg(:,:,:,:,:)
        INTEGER, INTENT (IN) :: gid
#if defined(__MPI)
...
!------------------------------------------------------------------------------!
!..env_mp_allgatherv_inplace_cplx_array
!..Ye Luo
...
!.. SdG added 16/08/19
      SUBROUTINE env_mp_allgatherv_inplace_real_array(alldata, my_element_type, recvcount, displs, gid)
        IMPLICIT NONE
        REAL(DP) :: alldata(:,:)
        INTEGER, INTENT(IN) :: my_element_type
        INTEGER, INTENT(IN) :: recvcount(:), displs(:)
...
#endif
        RETURN
      END SUBROUTINE env_mp_allgatherv_inplace_real_array
...
!------------------------------------------------------------------------------!
!..env_mp_circular_shift_left_start
SUBROUTINE env_mp_circular_shift_left_start_i0( sendbuf, recvbuf, itag, gid, requests)
   IMPLICIT NONE
...
#endif
   RETURN
END SUBROUTINE env_mp_circular_shift_left_start_i0


SUBROUTINE env_mp_circular_shift_left_start_i1( sendbuf, recvbuf, itag, gid, requests)
   IMPLICIT NONE
   INTEGER  :: sendbuf( : ), recvbuf( : )
   INTEGER, INTENT(IN) :: itag
   INTEGER, INTENT(IN) :: gid
   ...
   group = gid
   !
   IF( size(sendbuf)/=size(recvbuf) ) CALL env_mp_stop(8099)
   CALL mpi_comm_size( group, npe, ierr )
   IF (ierr/=0) CALL env_mp_stop( 8100 )
   CALL mpi_comm_rank( group, mype, ierr )
   IF (ierr/=0) CALL env_mp_stop( 8101 )
...
#endif
   RETURN
END SUBROUTINE env_mp_circular_shift_left_start_i1


SUBROUTINE env_mp_circular_shift_left_start_i2( sendbuf, recvbuf, itag, gid, requests)
   IMPLICIT NONE
   INTEGER  :: sendbuf( :, : ), recvbuf( :, : )
   INTEGER, INTENT(IN) :: itag
   INTEGER, INTENT(IN) :: gid
...
#endif
   RETURN
END SUBROUTINE env_mp_circular_shift_left_start_i2


SUBROUTINE env_mp_circular_shift_left_start_r2d( sendbuf, recvbuf, itag, gid, requests)
   IMPLICIT NONE
...
#endif
   RETURN
END SUBROUTINE env_mp_circular_shift_left_start_r2d


SUBROUTINE env_mp_circular_shift_left_start_c2d( sendbuf, recvbuf, itag, gid, requests)
   IMPLICIT NONE
...
#endif
   RETURN
END SUBROUTINE env_mp_circular_shift_left_start_c2d
!
!
!------------------------------------------------------------------------------!
!..env_mp_count_nodes
...
SUBROUTINE env_mp_type_create_real_column_section(dummy, start, length, stride, mytype)
  IMPLICIT NONE
  !
  ...
  CALL MPI_Type_commit(mytype, ierr)
  IF (ierr/=0) CALL env_mp_stop( 8084 )
#else
  mytype = 0;
#endif
  !
  RETURN
END SUBROUTINE env_mp_type_create_real_column_section

SUBROUTINE env_mp_type_create_cplx_row_section(dummy, column_start, column_stride, row_length, mytype)
  IMPLICIT NONE
  !
  COMPLEX (DP), INTENT(IN) :: dummy
  ...
  IF (ierr/=0) CALL env_mp_stop( 8085 )
  ...
  IF (ierr/=0) CALL env_mp_stop( 8086 )
  CALL MPI_Type_commit(mytype, ierr)
  IF (ierr/=0) CALL env_mp_stop( 8086 )
#else
  mytype = 0;
  ...
  !
  RETURN
END SUBROUTINE env_mp_type_create_cplx_row_section

SUBROUTINE env_mp_type_create_real_row_section(dummy, column_start, column_stride, row_length, mytype)
  IMPLICIT NONE
  !
  REAL (DP), INTENT(IN) :: dummy
  INTEGER, INTENT(IN) :: column_start, column_stride, row_length
  ...
  IF (ierr/=0) CALL env_mp_stop( 8087 )
  CALL MPI_TYPE_GET_EXTENT(MPI_DOUBLE_PRECISION, lb, extent, ierr)
  IF (ierr/=0) CALL env_mp_stop( 8087 )
  CALL MPI_TYPE_COMMIT(temporary, ierr)
  IF (ierr/=0) CALL env_mp_stop( 8087 )
  CALL MPI_TYPE_CREATE_RESIZED(temporary, lb, extent, mytype, ierr)
  IF (ierr/=0) CALL env_mp_stop( 8088 )
  CALL MPI_Type_commit(mytype, ierr)
  IF (ierr/=0) CALL env_mp_stop( 8088 )
#else
  mytype = 0;
  ...
  !
  RETURN
END SUBROUTINE env_mp_type_create_real_row_section
...
SUBROUTINE env_mp_type_free(mytype)
  ...
  IF (ierr/=0) CALL env_mp_stop( 8089 )
#endif
  !
  RETURN
...
!..env_mp_allgatherv_inplace_real_array
!

      SUBROUTINE env_mp_allgatherv_inplace_real_array_gpu(alldata_d, my_element_type, recvcount, displs, gid)
         IMPLICIT NONE
         REAL(DP), DEVICE :: alldata_d(:,:)
         INTEGER, INTENT(IN) :: my_element_type
         INTEGER, INTENT(IN) :: recvcount(:), displs(:)
         ...
         !
         ALLOCATE(alldata_h, source=alldata_d)! This syncs __MPI
         CALL env_mp_allgatherv_inplace_real_array(alldata_h, my_element_type, recvcount, displs, gid)
         alldata_d = alldata_h; DEALLOCATE(alldata_h)
         ierr = cudaDeviceSynchronize()   ! This syncs in case of small data chunks
         RETURN
#else
...
#endif
         ierr = cudaDeviceSynchronize()   ! This syncs SERIAL, __MPI
      END SUBROUTINE env_mp_allgatherv_inplace_real_array_gpu
      ...
      SUBROUTINE env_mp_type_create_real_column_section_gpu(dummy, start, length, stride, mytype)
         IMPLICIT NONE
         !
         ...
         !
         RETURN
      END SUBROUTINE env_mp_type_create_real_column_section_gpu

      SUBROUTINE env_mp_type_create_cplx_row_section_gpu(dummy, column_start, column_stride, row_length, mytype)
         IMPLICIT NONE
         !
         COMPLEX (DP), DEVICE, INTENT(IN) :: dummy
         INTEGER, INTENT(IN) :: column_start, column_stride, row_length
         ...
         !
         RETURN
      END SUBROUTINE env_mp_type_create_cplx_row_section_gpu

      SUBROUTINE env_mp_type_create_real_row_section_gpu(dummy, column_start, column_stride, row_length, mytype)
         IMPLICIT NONE
         !
         REAL (DP), DEVICE, INTENT(IN) :: dummy
         INTEGER, INTENT(IN) :: column_start, column_stride, row_length
         ...
         !
         RETURN
      END SUBROUTINE env_mp_type_create_real_row_section_gpu
...
!       global i
!       i += 1
!       return 'env_mp_stop( {0} )'.format(i)
!   
!   with open(sys.argv[1],'r') as f:
!       data = re.sub(r"env_mp_stop\(\s?\d+\s?\)", replace, f.read())
